...
public class Online_evProc_ipOspf extends IEventProcessor {
    ...
    public void initialize(NetPlan initialNetPlan, Map<String, String> algorithmParameters, Map<String, String> simulationParameters, Map<String, String> net2planParameters) {
        ...
        linkIGPWeightSetting.assign(initialNetPlan.getVectorLinkUpState(ipLayer), new DoubleDoubleFunction() {
            public double apply(double x, double y) {
                return y == 1 ? x : Double.MAX_VALUE;
            }
        });
    ...
    public void processEvent(NetPlan currentNetPlan, SimEvent event) {
        ...
        if (ipMaximumE2ELatencyMs.getDouble() > 0) for (Demand d : currentNetPlan.getDemands(ipLayer))
            if (d.getWorseCasePropagationTimeInMs() > ipMaximumE2ELatencyMs.getDouble())
                stat_trafficOutOfLatencyLimit += timeSinceLastChange * d.getOfferedTraffic();
...
//			System.out.print ("IPOSPF DemandModify: demand " + d + ", " + (ev.modificationIsRelativeToCurrentOfferedTraffic? "RELATIVE" : "ABSOLUTE") + " , old demand offered: " + d.getOfferedTraffic() + " ");
        ...
        linkIGPWeightSetting.assign(currentNetPlan.getVectorLinkUpState(ipLayer), new DoubleDoubleFunction() {
            public double apply(double x, double y) {
                return y == 1 ? x : Double.MAX_VALUE;
            }
        });
