 ...
 * <p>
 * The events generated targeted to the event processor module (e.g. {@code Online_evProc_wdm}) are:
 * <ul>
 * <li>WDMUtils.LightpathAdd: Request to add a new lightpath to the network. In this case, the lightpath is associated to an existing demand, which generates lightpath requests</li>
 ...
 * </ul>
 * The average rate of lightpath requests sent can change along time using fast and/or slow fluctuations. Fast fluctuations are intended to reflect typical short time-scale
...
public class Online_evGen_wdm extends IEventGenerator {
    ...
    public void initialize(NetPlan initialNetPlan, Map<String, String> algorithmParameters, Map<String, String> simulationParameters, Map<String, String> net2planParameters) {
        ...
        if (_tfSlow_fluctuationType.getString().equalsIgnoreCase("time-zone-based")) {
            ...
            try {
                this.tfSlow_calendar.setTime(new SimpleDateFormat(DATE_FORMAT).parse(tfSlow_startDate.getString()));
            } catch (Exception e) {
                e.printStackTrace();
                throw new Net2PlanException("Error parsing the date");
            }
            ...
            for (Node node : initialNetPlan.getNodes()) {
                ...
                if (timezone < -12 || timezone > 12)
                    throw new Net2PlanException(String.format("Timezone for node %d must be in range [-12, 12]", node.getIndex()));
        ...
        if (!_fail_failureModel.getString().equalsIgnoreCase("none")) {
            ...
            if (!fail_statisticalPattern.getString().equalsIgnoreCase("exponential-iid"))
                throw new Net2PlanException("Unknown failure statisitical pattern");
    ...
    public void processEvent(NetPlan currentNetPlan, SimEvent event) {
        ...
        if (this.cac_auxIncremental) {
            for (Pair<WDMUtils.LightpathAdd, Double> ev : new LinkedList<Pair<WDMUtils.LightpathAdd, Double>>(this.cacIncremental_potentiallyBlockedRouteRequests)) {
                ...
                else if (ev.getSecond() < simTime)
                    endSimulation(); // not assigned route, and it is in the past => end simulation in the incremental mode
            }
        }
...
//			scheduleEvent(new SimEvent (simTime, SimEvent.DestinationModule.EVENT_PROCESSOR , -1 , demandOfferedTrafficUpdate));
            ...
            if (tfFast_auxRandomGaussian) {
                ...
                throw new Net2PlanException("Unknow fast traffic fluctuation type: " + _tfFast_fluctuationType.getString());
            ...
            Set<Node> nodesUpToDown = new HashSet<Node>(currentNetPlan.getNodesUp());
            nodesUpToDown.retainAll(srg.getNodes());
            Set<Link> linksUpToDown = new HashSet<Link>(currentNetPlan.getLinksUpAllLayers());
            linksUpToDown.retainAll(srg.getLinks());
            ...
            Set<Node> nodesDownToUp = new HashSet<Node>(currentNetPlan.getNodesDown());
            nodesDownToUp.removeAll(nodesDownAfterRepair);
            Set<Link> linksDownToUp = new HashSet<Link>(currentNetPlan.getLinksDownAllLayers());
            linksDownToUp.removeAll(linksDownAfterRepair);
    ...
    private static class GenerateConnectionRequest {
        ...
        public GenerateConnectionRequest(Demand demand) {
            this.demand = demand;
        }

        @Override
        public String toString() {
            return "Generate connection request for demand " + demand.getId();
        }
    }
    ...
    private static class GenerateConnectionRelease {
        ...
        public GenerateConnectionRelease(WDMUtils.LightpathAdd routeAddEvent) {
            this.routeAddEvent = routeAddEvent;
        }

        @Override
        public String toString() {
            return "Generate connection release for demand " + routeAddEvent.demand.getId();
        }
    }
    ...
    private static class GenerateDemandOfferedTrafficFastFluctuation {
        ...
        public GenerateDemandOfferedTrafficFastFluctuation(Demand demand) {
            this.demand = demand;
        }

        @Override
        public String toString() {
            return "Generate fast fluctuation of offered traffic of demand " + demand.getId();
        }
    }
    ...
    private static class GenerateDemandOfferedTrafficSlowFluctuation {
        ...
        public GenerateDemandOfferedTrafficSlowFluctuation(Demand demand) {
            this.demand = demand;
        }

        @Override
        public String toString() {
            return "Generate slow fluctuation of offered traffic of demand " + demand.getId();
        }
    }
    ...
    private static class GenerateFailureSRG {
        ...
        public GenerateFailureSRG(SharedRiskGroup srg) {
            this.srg = srg;
        }

        @Override
        public String toString() {
            return "Generate failure in SRG " + srg.getId();
        }
    }
    ...
    private static class GenerateRepairSRG {
        ...
        public GenerateRepairSRG(SharedRiskGroup srg) {
            this.srg = srg;
        }

        @Override
        public String toString() {
            return "Generate repair event in SRG " + srg.getId();
        }
    }
