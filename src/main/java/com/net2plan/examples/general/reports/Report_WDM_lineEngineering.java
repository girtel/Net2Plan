    ...
    private LinkedList<String> computeWarningMessages(LinkedList<Triple<Double, String, Double>> elementPositions, LinkedList<Pair<double[], double[]>> impairmentsAtInputAndOutputs) {
        ...
        while (it_elementPositions.hasNext()) {
            ...
            if (name.equalsIgnoreCase("OADM-ADD")) {
                ...
                if (channels_minChannelLambda_nm < tp_minWavelength_nm)
                ...
                if (channels_maxChannelLambda_nm > tp_maxWavelength_nm)
                ...
                if (Math.abs(post_powerPerChannel_dBm - oadm_perChannelOutputPower_dBm) > 1E-3)
                ...
                if (pre_powerPerChannel_dBm < oadm_perChannelMinInputPower_dBm - 1E-3)
                ...
                if (pre_powerPerChannel_dBm > oadm_perChannelMaxInputPower_dBm + 1E-3)
                    st += "At " + initialPosition_km + "km: Power at the OADM-EXPRESS input is " + pre_powerPerChannel_dBm + " dBm. It should be between [" + oadm_perChannelMinInputPower_dBm + ", " + oadm_perChannelMaxInputPower_dBm + "] dBm";
				...
				/* Output power within limits */
                if (post_powerPerChannel_dBm < tp_inputPowerSensitivityMin_dBm - 1E-3)
                ...
                if (post_powerPerChannel_dBm > tp_inputPowerSensitivityMax_dBm + 1E-3)
                    st += "At " + initialPosition_km + "km: Power at the OADM-DROP output is " + post_powerPerChannel_dBm + ". It should be between [" + tp_inputPowerSensitivityMin_dBm + "," + tp_inputPowerSensitivityMax_dBm + "] dBm";

				/* Chromatic dispersion in the receiver */
                if (post_CD_ps_per_nm > tp_maxChromaticDispersionTolerance_ps_per_nm)
				...
				/* OSNR within limits */
                if (linear2dB(post_OSNR_linear) < tp_minOSNR_dB + osnrPenalty_SUM_dB)
                    st += "At " + initialPosition_km + "km: OSNR at the RECEIVER is " + linear2dB(post_OSNR_linear) + " dB. It is below the tolerance plus margin " + tp_minOSNR_dB + " dB " + osnrPenalty_SUM_dB + " dB = " + (tp_minOSNR_dB + osnrPenalty_SUM_dB) + " dB)";

				/* PMD tolerance at the receiver */
                final double pmdAtReceiver = Math.sqrt(post_PMDSquared_ps2);
                if (pmdAtReceiver > tp_pmdTolerance_ps)
                ...
                if (channels_minChannelLambda_nm < edfa_minWavelength_nm)
                ...
                if (channels_maxChannelLambda_nm > edfa_maxWavelength_nm)
				...
				/* Gain within limits */
                if (edfaGain_dB < edfa_minGain_dB - 1E-3)
                    st += "At " + initialPosition_km + "km: EDFA gain is " + edfaGain_dB + " dB. It should be between [" + edfa_minGain_dB + ", " + edfa_maxGain_dB + "] dB";
                if (edfaGain_dB > edfa_maxGain_dB + 1E-3)
                    st += "At " + initialPosition_km + "km: EDFA gain is " + edfaGain_dB + " dB. It should be between [" + edfa_minGain_dB + ", " + edfa_maxGain_dB + "] dB";

				/* Input power within limits */
                if (pre_powerPerChannel_dBm < edfa_minInputPower_dBm - 1E-3)
                ...
                if (pre_powerPerChannel_dBm + numChannels_dB > edfa_maxInputPower_dBm + 1E-3)
                    st += "At " + initialPosition_km + "km: Power at the EDFA input is (if all WDM channels are active) " + (pre_powerPerChannel_dBm + numChannels_dB) + " dBm. It should be between [" + edfa_minInputPower_dBm + "," + edfa_maxInputPower_dBm + "] dBm";

				/* Output power within limits */
                if (post_powerPerChannel_dBm < edfa_minOutputPower_dBm - 1E-3)
                ...
                if (post_powerPerChannel_dBm + numChannels_dB > edfa_maxOutputPower_dBm + 1E-3)
                    st += "At " + initialPosition_km + "km: Power at the EDFA output is (if all WDM channels are active) " + (post_powerPerChannel_dBm + numChannels_dB) + " dBm. It should be between [" + edfa_minOutputPower_dBm + ", " + edfa_maxOutputPower_dBm + "] dBm";
            } else if (name.equalsIgnoreCase("DCM")) {
                final double dcmCompensation_ps_per_nm = elementData;
                if ((dcmCompensation_ps_per_nm < dcm_channelDispersionMax_ps_per_nm) || (dcmCompensation_ps_per_nm > dcm_channelDispersionMin_ps_per_nm))
