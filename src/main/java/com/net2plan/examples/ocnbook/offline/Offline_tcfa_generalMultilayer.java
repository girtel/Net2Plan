    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        if (netPlan.isSingleLayer()) {
            ...
            for (Demand d : netPlan.getDemands(lowerLayer))
                netPlan.addDemand(d.getIngressNode(), d.getEgressNode(), d.getOfferedTraffic(), null, upperLayer);
        } else {
        ...
        for (Link eHi : netPlan.getLinks(upperLayer))
            if (Math.abs(eHi.getCapacity() - ciurcuitCapacityGbps.getDouble() * z_c.get(eHi.getIndex())) > 1e-3)
                throw new RuntimeException("Bad");
        netPlan.removeAllLinksUnused(PRECISION_FACTOR, upperLayer);

		/* check */
        if (netPlan.getVectorDemandBlockedTraffic(upperLayer).zSum() > PRECISION_FACTOR)
            throw new RuntimeException("Bad: " + netPlan.getVectorDemandBlockedTraffic(upperLayer));
        if (netPlan.getVectorDemandBlockedTraffic(lowerLayer).zSum() > PRECISION_FACTOR)
            throw new RuntimeException("Bad");
        if (netPlan.getVectorLinkOversubscribedTraffic(upperLayer).zSum() > PRECISION_FACTOR)
            throw new RuntimeException("Bad");
        if (netPlan.getVectorLinkOversubscribedTraffic(lowerLayer).zSum() > PRECISION_FACTOR)
            throw new RuntimeException("Bad");
