...
public class Offline_cfa_xpMultiperiodModularCapacities implements IAlgorithm {
    ...
    private InputParameter solverName = new InputParameter("solverName", "#select# glpk cplex", "The solver name to be used by JOM. GLPK is free, CPLEX commercial. Both GLPK and CPLEX solve linear problems w/w.o integer contraints.");
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        if (c_k0.size() != K)
            throw new Net2PlanException("The number of costs should be equal to the number of types of capacity modules");
        ...
        while (true) {
            try {
                ...
                if (thisIntervalTrafficMatrix.rows() != N)
                    throw new Net2PlanException("The number of nodes in traffic matrix: " + rootOfNameOfInputTrafficFiles.getString() + "_tm" + netPlanFiles.size() + ".n2p (" + thisIntervalTrafficMatrix.rows() + ") is not correct (" + N + ")");
                NetPlan netPlanToAdd = netPlan.copy();
                for (Demand d : netPlanToAdd.getDemands())
                    d.setOfferedTraffic(thisIntervalTrafficMatrix.get(d.getIngressNode().getIndex(), d.getEgressNode().getIndex()));
        ...
        DoubleMatrix2D timeAccumulationMatrix = DoubleFactory2D.dense.make(T, T);
        for (int t1 = 0; t1 < T; t1++) for (int t2 = t1; t2 < T; t2++) timeAccumulationMatrix.set(t1, t2, 1.0);
        ...
        for (int t = 0; t < T; t++) {
            ...
            for (Link link : thisNp.getLinks()) {
                ...
                double linkCapacityAccumulatingPreviosModules = 0;
                for (int t1 = 0; t1 <= t; t1++)
                    for (int k = 0; k < K; k++)
                        linkCapacityAccumulatingPreviosModules += u_k.get(k) * a_ket.get(k, e, t1);
            ...
            if (thisNp.getVectorLinkOversubscribedTraffic().zSum() > PRECISION_FACTOR)
                throw new RuntimeException("Bad: " + thisNp.getVectorLinkOversubscribedTraffic().zSum());
            if (thisNp.getDemandTotalBlockedTraffic() > PRECISION_FACTOR)
                throw new RuntimeException("Bad: " + thisNp.getDemandTotalBlockedTraffic());
        }
