...
public class Offline_cba_wirelessCongControlTransmissionPowerAssignment implements IAlgorithm {
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        for (int e = 0; e < E; e++) {
            G_ee.set(e, e, mac_g_nu_ee.get(e, e));
            G_eep.set(e, e, 0);
        }
        ...
        for (Link e : netPlan.getLinks()) {
            ...
            if (e.getCapacity() < e.getCarriedTrafficIncludingProtectionSegments() - PRECISIONFACTOR)
                throw new RuntimeException("Bad");
        }

        boolean allMaxPower = true;
        for (Link e : netPlan.getLinks())
            if (Math.abs(mac_maxTransmissionPower_logu.getDouble() - Double.parseDouble(e.getAttribute("p_e"))) > 1E-3)
                allMaxPower = false;
    ...
    private double computeSINR_e(Link e, DoubleMatrix1D p_e, DoubleMatrix2D mac_g_nu_ee, double mac_receptionThermalNoise_nu, NetPlan np) {
        ...
        for (Link eInt : np.getLinks())
            if (eInt != e)
                interferencePower_nu += Math.exp(p_e.get(eInt.getIndex())) * mac_g_nu_ee.get(eInt.getIndex(), e.getIndex());
