...
public class Offline_fa_xdeFormulationsMulticast implements IAlgorithm {
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        if (E == 0 || MD == 0)
            throw new Net2PlanException("This algorithm requires a topology with links and a multicast demand set");
...
//		op.setInputParameter("I_eet", new DoubleMatrixND (I_eet));
        DoubleMatrixND I_eet = new DoubleMatrixND(new int[]{E, E, N}, "sparse");
        for (int t = 0; t < N; t++) for (int e = 0; e < E; e++) I_eet.set(new int[]{e, e, t}, 1.0);
        ...
        if (maxE2ELengthConstraintApplies)
            op.addConstraint("lengthInKm_e * permute(xx_det,[2;1;3]) <= " + maxE2ELengthInKm.getDouble()); // maximum length in km from ingress node to any demand egress node is limited
        if (maxE2ENumHopsConstraintApplies)
            op.addConstraint("sum(xx_det,2) <= " + maxE2ENumHops.getInt()); // maximum number of hops from ingress node to any demand egress node is limited
        if (maxE2EPropDelayConstraintApplies)
            op.addConstraint("propDelay_e * permute(xx_det,[2;1;3])  <= " + maxE2EPropDelayInMs.getDouble()); // maximum propagation delay in ms from ingress node to any demand egress node is limited
            ...
            for (Link e : netPlan.getLinks())
                for (Node n : netPlan.getNodes())
                    if (xx_de.get(d.getIndex(), e.getIndex()) == 0)
                        if (xx_det.get(new int[]{d.getIndex(), e.getIndex(), n.getIndex()}) != 0)
                            throw new RuntimeException("Bad: x_de: " + (xx_de.get(d.getIndex(), e.getIndex())) + ", xx_det: " + (xx_det.get(new int[]{d.getIndex(), e.getIndex(), n.getIndex()})));

        for (MulticastDemand d : netPlan.getMulticastDemands()) {
            Set<Link> linkSet = new HashSet<Link>();
            for (int e = 0; e < E; e++)
                if (xx_de.get(d.getIndex(), e) != 0) {
                    linkSet.add(netPlan.getLink(e));
                }
