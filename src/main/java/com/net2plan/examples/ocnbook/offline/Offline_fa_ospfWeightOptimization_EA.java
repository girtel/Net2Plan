...
public class Offline_fa_ospfWeightOptimization_EA implements IAlgorithm {
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        if (ea_offSpringSize.getInt() > ea_populationSize.getInt() / 2.0)
            throw new Net2PlanException("The offspring size cannot exceed the population size divided by two");
    ...
    private List<DoubleMatrix1D> operator_crossover(LinkedList<Integer> parents, Random rng) {
        ...
        while (parents.size() >= 2) {
            ...
            for (Link link : netPlan.getLinks())
                descendant.set(link.getIndex(), rng.nextBoolean() ? firstParent.get(link.getIndex()) : secondParent.get(link.getIndex()));
    ...
    private double computeAverageEntropy() {
        ...
        for (int eIndex = 0; eIndex < E; eIndex++) {
            ...
            for (int w = 0; w < ospf_maxLinkWeight.getInt(); w++) {
                if (sum_e[eIndex] > 0) {
                    final double val = freq_ew[eIndex][w] / sum_e[eIndex];
                    entropy_e -= (val == 0) ? 0 : val * Math.log(val) * convLogFactor;
                }
            }
            avEntropy += entropy_e;
        }
