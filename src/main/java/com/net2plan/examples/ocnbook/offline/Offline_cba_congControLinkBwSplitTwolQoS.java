    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        for (Node n1 : netPlan.getNodes())
            for (Node n2 : netPlan.getNodes())
                if (n1 != n2) {
                    final Demand d1 = netPlan.addDemand(n1, n2, 0.0, null);
                    d1.setAttribute("type", "1");
                    demandType.set(d1.getIndex(), 1);
                    final Demand d2 = netPlan.addDemand(n1, n2, 0.0, null);
                    d2.setAttribute("type", "2");
        ...
        for (Link e : netPlan.getLinks()) {
            ...
            for (Route r : e.getTraversingRoutes())
                if (demandType.get(r.getDemand().getIndex()) == 1) traf1 += r.getCarriedTraffic();
                else traf2 += r.getCarriedTraffic();
            ...
            if (u_1.get(e.getIndex()) + u_2.get(e.getIndex()) > e.getCapacity() + PRECISIONFACTOR)
