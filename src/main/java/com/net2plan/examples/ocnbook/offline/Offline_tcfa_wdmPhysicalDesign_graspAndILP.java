...
public class Offline_tcfa_wdmPhysicalDesign_graspAndILP implements IAlgorithm {
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
		...
		/* Initialize links */
            for (Node n1 : netPlan.getNodes())
                for (Node n2 : netPlan.getNodes())
                    if (n1 != n2)
                        netPlan.addLink(n1, n2, 0.01, netPlan.getNodePairEuclideanDistance(n1, n2), 200000, null);
            ...
            for (Link e : netPlan.getLinks())
                e.setLengthInKm(e.getLengthInKm() * linkLengthKmFromEuclideanDistanceFactor);
            this.Efm = netPlan.getNumberOfLinks();
            if (Efm != N * (N - 1)) throw new RuntimeException("Bad, Efm" + Efm + ", N: " + N);
            this.linkCost_e = DoubleFactory1D.dense.make(Efm);
            for (Link e : netPlan.getLinks())
                linkCost_e.set(e.getIndex(), tcfa_linkCostPerKm.getDouble() * e.getLengthInKm());
		
		/* If there is no traffic, create uniform traffic matrix */
            if (netPlan.getNumberOfDemands() == 0)
                for (Node n1 : netPlan.getNodes())
                    for (Node n2 : netPlan.getNodes())
                        if (n1 != n2) netPlan.addDemand(n1, n2, tcfa_circuitCapacity_Gbps.getDouble(), null);
		...
		/* If 1+1 then decouple demands in one per channel */
            if (tcfa_recoveryType.getString().equals("1+1")) {
                ...
                for (Demand d : npcopy.getDemands()) {
                    ...
                    for (int cont = 0; cont < numChannels; cont++)
                        netPlan.addDemand(netPlan.getNodeFromId(d.getIngressNode().getId()), netPlan.getNodeFromId(d.getEgressNode().getId()), tcfa_circuitCapacity_Gbps.getDouble(), d.getAttributes());
                }
            }
		...
		/* Initialize SRG information */
            ...
            this.routeList_d = new int[D][];
            for (Demand d : netPlan.getDemands()) routeList_d[d.getIndex()] = getIndexes(d.getRoutes());
            this.traversedLinks_r = new int[R][];
            for (Route r : netPlan.getRoutes()) traversedLinks_r[r.getIndex()] = getIndexes(r.getSeqLinksRealPath());
		
		
		/* Initialize aux arrays for speed-up */
            this.A_dr = netPlan.getMatrixDemand2RouteAssignment();
            for (Demand d : netPlan.getDemands())
                if (d.getRoutes().isEmpty()) throw new Net2PlanException("A demand has no assigned routes");
            ...
            for (Route r : netPlan.getRoutes())
                for (Link e : r.getSeqLinksRealPath())
                    for (SharedRiskGroup srg : e.getSRGs())
		...
		/* Check if the problem may have a solution: demands have at least one path in any failure state  */
            if (A_dr.zMult(A_rs, null).getMinLocation()[0] == 0) {
                System.out.println("A_dr.zMult(A_rs, null): " + A_dr.zMult(A_rs, null));
                throw new Net2PlanException("Some demands cannot be routed in some failure state. We need more paths! (e.g. extend the path reach)");
            }

            this.Aout_ne = netPlan.getMatrixNodeLinkOutgoingIncidence();
            this.Ain_ne = netPlan.getMatrixNodeLinkIncomingIncidence();
            this.Abid_dd = DoubleFactory2D.sparse.make(D, D);
            for (Entry<Demand, Demand> entry : opposite_d.entrySet()) {
                Abid_dd.set(entry.getKey().getIndex(), entry.getValue().getIndex(), 1.0);
                Abid_dd.set(entry.getValue().getIndex(), entry.getKey().getIndex(), 1.0);
            }
            this.Abid_rr = DoubleFactory2D.sparse.make(R, R);
            for (Entry<Route, Route> entry : opposite_r.entrySet()) {
                Abid_rr.set(entry.getKey().getIndex(), entry.getValue().getIndex(), 1.0);
                Abid_rr.set(entry.getValue().getIndex(), entry.getKey().getIndex(), 1.0);
            }
            this.Abid_ee = DoubleFactory2D.sparse.make(Efm, Efm);
            for (Link e : netPlan.getLinks()) {
                Abid_ee.set(e.getIndex(), oppositeLink(e).getIndex(), 1.0);
                Abid_ee.set(oppositeLink(e).getIndex(), e.getIndex(), 1.0);
            }
		...
		/* Up stage */
            ArrayList<Integer> shuffledDemands = new ArrayList<Integer>(D);
            for (int d = 0; d < D; d++) shuffledDemands.add(d);
            Collections.shuffle(shuffledDemands, rng);
            if (tcfa_recoveryType.getString().equals("shared")) {
                DoubleMatrix1D best_xr = null;
                DoubleMatrix1D best_pe = null;
                ...
                while ((System.nanoTime() < algorithmEndtime) && (iterationCounter < tcfa_maxNumIterations.getInt())) {
                    ...
                    DoubleMatrix1D thisIt_pe = A_er.zMult(thisIt_xr, null);
                    thisIt_pe.assign(DoubleFunctions.div(tcfa_linkCapacity_numCirc.getInt())).assign(DoubleFunctions.ceil);
                    double thisItCost = computeCost_shared(thisIt_xr, thisIt_pe);
                    if (thisItCost < bestCost) {
                        best_xr = thisIt_xr;
                        best_pe = thisIt_pe;
                        bestCost = thisItCost;
                    }
                ...
                for (Route r : netPlan.getRoutes())
                    r.setCarriedTraffic(tcfa_circuitCapacity_Gbps.getDouble() * best_xr.get(r.getIndex()), tcfa_circuitCapacity_Gbps.getDouble() * best_xr.get(r.getIndex()));
                for (Link e : netPlan.getLinks())
                    e.setCapacity(tcfa_linkCapacity_numCirc.getInt() * tcfa_circuitCapacity_Gbps.getDouble() * best_pe.get(e.getIndex()));
            } else if (tcfa_recoveryType.getString().equals("1+1")) {
                DoubleMatrix1D best_xr = null;
                DoubleMatrix1D best_x2r = null;
                DoubleMatrix1D best_pe = null;
                double bestCost = Double.MAX_VALUE;
                int iterationCounter = 0;
                while ((System.nanoTime() < algorithmEndtime) && (iterationCounter < tcfa_maxNumIterations.getInt())) {
                ...
                stat_numItGRASP = iterationCounter;
                for (Demand d : netPlan.getDemands()) {
                    if (Math.abs(best_xr.viewSelection(routeList_d[d.getIndex()]).zSum() - 1) > 1E-3)
                        throw new RuntimeException("Bad");
                    if (Math.abs(best_x2r.viewSelection(routeList_d[d.getIndex()]).zSum() - 1) > 1E-3)
                        throw new RuntimeException("Bad");
                    Route primaryRoute = null;
                    ProtectionSegment protSegment = null;
                    for (Route r : d.getRoutes())
                        if (Math.abs(best_xr.get(r.getIndex()) - 1) <= 1e-3) {
                            primaryRoute = r;
                            primaryRoute.setCarriedTraffic(tcfa_circuitCapacity_Gbps.getDouble(), tcfa_circuitCapacity_Gbps.getDouble());
                            break;
                        }
                    for (Route r : d.getRoutes())
                        if (Math.abs(best_x2r.get(r.getIndex()) - 1) <= 1e-3) {
                            protSegment = netPlan.addProtectionSegment(r.getSeqLinksRealPath(), tcfa_circuitCapacity_Gbps.getDouble(), null);
                            break;
                        }
                ...
                stat_numItGRASP = iterationCounter;
                for (Route r : netPlan.getRoutes())
                    r.setCarriedTraffic(tcfa_circuitCapacity_Gbps.getDouble() * best_xrs.get(r.getIndex(), 0), tcfa_circuitCapacity_Gbps.getDouble() * best_xrs.get(r.getIndex(), 0));
                for (Link e : netPlan.getLinks())
                    e.setCapacity(tcfa_linkCapacity_numCirc.getInt() * tcfa_circuitCapacity_Gbps.getDouble() * best_pe.get(e.getIndex()));
		...
		/* Remove unused routes and links */
            ...
            stat_numCirc = (int) Math.round(netPlan.getDemandTotalCarriedTraffic() / tcfa_circuitCapacity_Gbps.getDouble());
            if (tcfa_recoveryType.getString().equals("1+1")) stat_numCirc *= 2;
            for (Node n : netPlan.getNodes())
                if (n.getOutgoingLinks().size() > 2) stat_numNodesDeg3++;
                else stat_numNodesDeg2++;
            ...
            try {
                ...
                e.printStackTrace();
                throw new RuntimeException("Not possible to write in File " + fileNameStem + "_allResults.txt");
            }
...
//		if (netPlan.getLinksOversubscribed().size() > 0) throw new RuntimeException ("Bad");
    ...
    private Link oppositeLink(Link e) {
        return e.getNetPlan().getNodePairLinks(e.getDestinationNode(), e.getOriginNode(), false).iterator().next();
    }
    ...
    private Pair<Map<Demand, Demand>, Map<Route, Route>> initializeNetPlanLinksBidirDemandsAndRoutes(NetPlan np) {
        ...
        np.removeAllRoutes();
        np.removeAllProtectionSegments();
        for (Node n1 : np.getNodes())
            for (Node n2 : np.getNodes())
                if (n1.getIndex() > n2.getIndex()) for (Demand d : np.getNodePairDemands(n1, n2, false)) d.remove();
        ...
        for (Demand d : new HashSet<Demand>(np.getDemands())) {
            ...
            for (Route r : new HashSet<Route>(d.getRoutes())) {
                ...
                opposite_r.put(r, oppRoute);
                opposite_r.put(oppRoute, r);
            }
        }
    ...
    private void checkSolution(NetPlan np) {
            ...
            for (Node n2 : np.getNodes())
                if (n1.getIndex() > n2.getIndex()) {
                    ...
                    if (tcfa_bidirectionalLinks.getBoolean()) {
                        ...
                        if (thisLinks.size() > 0)
                            if (thisLinks.iterator().next().getCapacity() != oppLinks.iterator().next().getCapacity())
                                throw new RuntimeException("Bad");
                    }
                }
        ...
        if (tcfa_recoveryType.getString().equals("1+1")) { // one prot segment, and link disjoint
            for (Route r : np.getRoutes()) {
    ...
    private DoubleMatrix1D jomStep_shared(Set<Integer> demandsChange, DoubleMatrix1D maxPe, DoubleMatrix1D x_r) {
        ...
        int[] rChange = new int[0];
        for (int d : dChange) rChange = IntUtils.concatenate(rChange, routeList_d[d]);
        ...
        for (int rRest = 0; rRest < Rrest; rRest++) {
            ...
            Abid_rRestrRest.set(rRest, oppRrest[0], 1.0);
            Abid_rRestrRest.set(oppRrest[0], rRest, 1);
        }
        ...
        if (tcfa_bidirectionalLinks.getBoolean()) {
            op.setInputParameter("Abid_ee", new DoubleMatrixND(Abid_ee));
            op.addConstraint("p_e == p_e * Abid_ee");
        }
        ...
        do {
            ...
            try {
                stat_numSolverCalls++;
                op.solve(algorithm_solverName.getString(), "solverLibraryName", algorithm_solverLibraryName.getString(), "maxSolverTimeInSeconds", solverTimeAllowed);
            } catch (Exception e) {
                System.out.println("-- EXTRA TIME FOR SOLVER, NOW: " + solverTimeAllowed);
                solverTimeAllowed += 2;
                stat_totalTimeSolverSecs += eTime(t);
                continue;
            }
            stat_totalTimeSolverSecs += eTime(t);
            if (solverTimeAllowed != algorithm_maxSolverTimeInSeconds.getDouble())
                System.out.println("-- EXTRA TIME FOR SOLVER, NOW: " + solverTimeAllowed);
...
//		System.out.println ("dChange :" + Arrays.toString(dChange));
        ...
        for (int e = 0; e < Efm; e++)
            occupiedWithoutChangingR_e.set(e, occupiedWithoutChangingR_e.get(e) + backup_occupiedWithoutChangingR_e.get(e));
        DoubleMatrix2D Abid_rRestrRest = DoubleFactory2D.sparse.make(Rrest, Rrest);
        for (int rRest = 0; rRest < Rrest; rRest++) {
            final int r = rChange[rRest];
            final int opp_r = opposite_r.get(netPlan.getRoute(r)).getIndex();
...
//		DoubleMatrix2D A_dRestrRest = A_dr.viewSelection(dChange, rChange);
        ...
        for (int rRest = 0; rRest < Rrest; rRest++) {
            final int r = rChange[rRest];
            final int dRest[] = IntUtils.find(dChange, netPlan.getRoute(r).getDemand().getIndex(), SearchType.ALL);
    ...
    private DoubleMatrix1D greedyAndLocalSearch_shared(long algorithmEndtime, NetPlan np) {
        ...
        for (Node n1 : np.getNodes())
            for (Node n2 : np.getNodes()) if (n1.getIndex() < n2.getIndex()) shuffledNodePairs.add(Pair.of(n1, n2));
...
//			System.out.println("thsiDemand: " + thisDemand);
        ...
        DoubleMatrix1D current_pe = A_er.zMult(current_xr, null);
        current_pe.assign(DoubleFunctions.div(tcfa_linkCapacity_numCirc.getInt())).assign(DoubleFunctions.ceil);
...
//				System.out.println("Pair: " + nodePair + ". Tabu list: " + tabuList_n1n2 + ", IS TABU: " + tabuList_n1n2.contains(nodePair));
                ...
                if (jomStep_shared(demandsToChange, null, thisNeighbor_xr) == null)
                    throw new RuntimeException("Returned null");
                DoubleMatrix1D thisNeighbor_pe = A_er.zMult(thisNeighbor_xr, null);
                thisNeighbor_pe.assign(DoubleFunctions.div(tcfa_linkCapacity_numCirc.getInt())).assign(DoubleFunctions.ceil);
                final double thisNeighborCost = computeCost_shared(thisNeighbor_xr, thisNeighbor_pe);
                if (thisNeighborCost < current_cost) {
                    current_xr = thisNeighbor_xr;
                    current_pe = thisNeighbor_pe;
                    current_cost = thisNeighborCost;
            ...
            else stat_numLSIterationsReducingCost++; // end grasp iteration
        }
    ...
    private Pair<DoubleMatrix1D, DoubleMatrix1D> greedyAndLocalSearch_11(long algorithmEndtime, NetPlan np) {
        ...
        for (Pair<Node, Node> nodePair : shuffledNodePairs) {
            ...
            for (Demand d : np.getNodePairDemands(nodePair.getFirst(), nodePair.getSecond(), true))
                thisDemand.add(d.getIndex());
            final long t = System.nanoTime();
            if (jomStep_11(thisDemand, null, current_xr, current_x2r) == null)
                throw new RuntimeException("Returned null");
        ...
        while (System.nanoTime() < algorithmEndtime) {
            ...
            for (Pair<Node, Node> nodePair : shuffledNodePairs) {
                ...
                if (jomStep_11(demandsToChange, null, thisNeighbor_xr, thisNeighbor_x2r) == null)
                    throw new RuntimeException("Returned null");
...
//			System.out.println("thsiDemand: " + thisDemand);
        ...
        while (System.nanoTime() < algorithmEndtime) {
            ...
            for (Pair<Node, Node> nodePair : shuffledNodePairs) {
                ...
                if (jomStep_restoration(demandsToChange, null, thisNeighbor_xrs) == null)
                    throw new RuntimeException("Returned null");
    ...
    private DoubleMatrix1D computePe_restoration(DoubleMatrix2D x_rs) {
        DoubleMatrix2D p_es = A_er.zMult(x_rs, null);
        DoubleMatrix1D p_e = DoubleFactory1D.dense.make(Efm);
        for (int e = 0; e < Efm; e++) p_e.set(e, p_es.viewRow(e).getMaxLocation()[0]); // the maximum per state s
        p_e.assign(DoubleFunctions.div(tcfa_linkCapacity_numCirc.getInt())).assign(DoubleFunctions.ceil);
        return p_e;
    }

    private double eTime(long init) {
        return 1E-9 * (System.nanoTime() - init);
    }

    private static int[] getIndexes(Collection<? extends NetworkElement> col) {
        final int[] res = new int[col.size()];
        int cont = 0;
        for (NetworkElement el : col) res[cont++] = el.getIndex();
        return res;
    }
