...
public class Offline_fa_ospfWeightOptimization_tabuSearch implements IAlgorithm {
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        if (tabuListSize >= E)
            throw new Net2PlanException("The tabu list size is larger or equal than the number of links: all jumps would be tabu");
        ...
        Map<Link, Integer> numEntriesTabuList_e = new HashMap<Link, Integer>();
        for (Link e : netPlan.getLinks()) numEntriesTabuList_e.put(e, 0);
        ...
        for (int eIndex = 0; eIndex < E; eIndex++)
            this.numberOccurrencies_ew[eIndex][(int) (currentSol.get(eIndex)) - 1]++;
        ...
        while ((System.nanoTime() < algorithmEndtime) && (numIterations < ts_maxNumIterations.getInt())) {
            ...
            for (Link e1 : netPlan.getLinks()) {
                ...
                for (int w1 = 1; w1 <= ospf_maxLinkWeight.getInt(); w1++) {
                    ...
                    if ((!isTabu && (neighborObjFunction < bestNeighborObjFunction)) || (isTabu && ts_aspirationCriterion.getBoolean() && (neighborObjFunction < bestObjFunction))) {
                        ...
                        bestNeighborLink = e1;
                        bestNeighborWeight = w1;
                        bestNeighborObjFunction = neighborObjFunction;
                    }
                }
            }

            if (bestNeighborLink == null) {
                throw new RuntimeException("This should not happen: it means a too large tabu list");
            }
			...
			/* update tabu list */
            if (tabuList_e.size() == tabuListSize) {
                Link eToRemove = tabuList_e.removeFirst();
                numEntriesTabuList_e.put(eToRemove, numEntriesTabuList_e.get(eToRemove) - 1);
                if (numEntriesTabuList_e.get(eToRemove) < 0) throw new RuntimeException("Bad");
            }
            tabuList_e.addLast(bestNeighborLink);
            numEntriesTabuList_e.put(bestNeighborLink, numEntriesTabuList_e.get(bestNeighborLink) + 1);
            ...
            if (currentObjFunction < bestObjFunctionSinceLastRandomization) {
                ...
                if (currentObjFunction < bestObjFunction) {
                    bestObjFunction = currentObjFunction;
                    bestSol = currentSol.copy();
                    System.out.println("Improving best objFunction: " + bestObjFunction);
                }
            } else
