...
public class Offline_ca_wirelessPersistenceProbability implements IAlgorithm {
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        for (Node n : netPlan.getNodes())
            if (minLinkPersistenceProb.getDouble() * n.getOutgoingLinks().size() > maxNodePersistenceProb.getDouble())
                throw new Net2PlanException("Minimum persistence per link is too high or maximum persistence per node too small");

		/* Take link nominal capacities */
        if (linkNominalCapacity.getDouble() > 0)
            netPlan.setVectorLinkCapacity(DoubleFactory1D.dense.make(E, linkNominalCapacity.getDouble()));
        ...
        DoubleMatrix1D initQn = DoubleFactory1D.dense.make(N);
        for (Node n : netPlan.getNodes())
            initQn.set(n.getIndex(), minLinkPersistenceProb.getDouble() * n.getOutgoingLinks().size());
...
//			op.setObjectiveFunction("maximize", "(1/b) * sum (    exp(  b*ln(nomU_e') +   b*ln(p_e') +   b*interf_en *ln(1-q_n)'   )      )");
        ...
        for (int n = 0; n < N; n++) {
            netPlan.getNode(n).setAttribute("q_n", "" + q_n[n]);
        }
