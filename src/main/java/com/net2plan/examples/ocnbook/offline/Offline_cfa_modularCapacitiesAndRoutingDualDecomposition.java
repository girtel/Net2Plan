    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        for (int n1 = 0; n1 < N; n1++)
            for (int n2 = 0; n2 < N; n2++)
                if (n1 == n2) continue;
                else trafficMatrix[n2][n2] -= trafficMatrix[n1][n2];
        ...
        for (int it = 1; it <= simulation_numIterations.getInt(); it++) {
            ...
            if (highestDualCost > lowestPrimalCost + PRECISIONFACTOR)
                throw new RuntimeException("Bad: highestDualCost: " + highestDualCost + ", lowestPrimalCost: " + lowestPrimalCost);
...
//		System.out.println("barU_e: " + barU_e);
    ...
    private void finish(NetPlan np) {
        ...
        for (Node s : np.getNodes()) {
            double outTraffic = 0;
            for (Demand d : s.getOutgoingDemands()) outTraffic += d.getOfferedTraffic();
            lowerBoundNumLps_1 += Math.ceil(outTraffic / moduleCapacity.getDouble());
        }
        double lowerBoundNumLps_2 = 0;
        for (Node t : np.getNodes()) {
            double inTraffic = 0;
            for (Demand d : t.getIncomingDemands()) inTraffic += d.getOfferedTraffic();
        ...
        for (Node t : np.getNodes())
            for (Node s : np.getNodes()) {
                ...
                for (Demand d : np.getNodePairDemands(s, t, false))
    ...
    void saveNetPlan(NetPlan netPlan, DoubleMatrix2D x_te, DoubleMatrix1D n_e) {
        ...
        for (Link e : netPlan.getLinksOversubscribed())
            if (e.getOccupiedCapacityIncludingProtectionSegments() - e.getCapacity() > PRECISIONFACTOR)
