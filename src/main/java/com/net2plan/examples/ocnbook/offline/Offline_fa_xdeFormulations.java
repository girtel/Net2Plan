...
public class Offline_fa_xdeFormulations implements IAlgorithm {
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        if (solverName.getString().equalsIgnoreCase("ipopt") && nonBifurcatedRouting.getBoolean())
            throw new Net2PlanException("With IPOPT solver, the routing cannot be constrained to be non-bifurcated");
        ...
        if (optimizationTarget.getString().equals("min-av-num-hops")) {
            ...
            if (maxLengthInKm.getDouble() > 0) {
                if (nonBifurcatedRouting.getBoolean())
                    throw new Net2PlanException("The maximum length constraint can only be applied in this formulation with non-bifurcated routing");
            ...
            if (!solverName.getString().equalsIgnoreCase("ipopt") || nonBifurcatedRouting.getBoolean())
                throw new Net2PlanException("This is a convex non linear model: please use IPOPT solver. The routing cannot be constrained to be non-bifurcated");
            if (maxLengthInKm.getDouble() > 0)
                throw new Net2PlanException("The maximum length constraint can not be applied in this objective function");
