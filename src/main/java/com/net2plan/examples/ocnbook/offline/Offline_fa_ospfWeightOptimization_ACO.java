...
public class Offline_fa_ospfWeightOptimization_ACO implements IAlgorithm {
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters) {
        ...
        double[][] pheromones_ew = new double[E][ospf_maxLinkWeight.getInt()];
        for (int e = 0; e < E; e++) Arrays.fill(pheromones_ew[e], 1.0);
        ...
        while ((System.nanoTime() < algorithmEndtime) && (iterationCounter < aco_maxNumIterations.getInt())) {
            ...
            for (Link e : netPlan.getLinks()) {
                ...
                for (int w = 0; w < pheromone_w.length; w++)
                    pheromone_w[w] *= 1 - aco_pheromoneEvaporationRate.getDouble();
            }
    ...
    private void antWeightChoice(Link e, DoubleMatrix1D currentSol, int maxLinkWeight, int differenceInWeightToBeNeighbors, double[] pheromone_w, double factorImportanceOfPheromones, Random rng) {
        ...
        for (int w = 0; w < accumProbabilities.length; w++) {
            accumProbabilities[w] = pheromone_w[w] * factorImportanceOfPheromones;
            accumProbabilitySum += accumProbabilities[w];
        }
    ...
    private double[][] copyOf(double[][] x) {
        ...
        for (int c1 = 0; c1 < x.length; c1++) res[c1] = Arrays.copyOf(x[c1], x[c1].length);
        return res;
    }

    private double[] copyOf(double[] x, int numValuesCopy, int finalSize) {
        double[] res = new double[finalSize];
        for (int c1 = 0; c1 < numValuesCopy; c1++) res[c1] = x[c1];
        return res;
    }
}