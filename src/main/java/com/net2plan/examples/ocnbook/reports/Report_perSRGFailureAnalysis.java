...
public class Report_perSRGFailureAnalysis implements IReport {
    ...
    public String executeReport(NetPlan netPlan, Map<String, String> reportParameters, Map<String, String> net2planParameters) {
        ...
        Set<Link> npForProducingAllFailures_linksAllLayers = new HashSet<Link>();
        for (NetworkLayer layer : npForProducingAllFailures.getNetworkLayers())
            npForProducingAllFailures_linksAllLayers.addAll(npForProducingAllFailures.getLinks(layer));
        ...
        for (int srgIndex = 0; srgIndex < netPlan.getNumberOfSRGs(); srgIndex++) {
            if (!npForProducingAllFailures.getLinksDownAllLayers().isEmpty() || !npForProducingAllFailures.getNodesDown().isEmpty())
                throw new RuntimeException("Bad");
    ...
    private String percentage(double val, double total) {
        double res = 100 * val / total;
        if (res < 1e-8) res = 0;
        return " (" + valString(res) + " %)";
    }

    private String percentage(int val, int total) {
        double res = 100 * ((double) val) / ((double) total);
        if (res < 1e-8) res = 0;
        return " (" + valString(res) + " %)";
    }

    private String valString(double traffic) {
        return String.format("%.3f", traffic < PRECISION_FACTOR ? 0 : traffic);
    }
