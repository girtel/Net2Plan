...
public class Report_availability implements IReport {
    ...
    public String executeReport(NetPlan netPlan, Map<String, String> reportParameters, Map<String, String> net2planParameters) {
        ...
        if (algorithmFile.isEmpty() || algorithmName.isEmpty())
        ...
        System.out.println("Probability no failure state: " + pi_s0);
...
//		System.out.println ("Links set to down all layers: " + netPlan.getLinksDownAllLayers());
        ...
        for (int indexLayer = 0; indexLayer < netPlan.getNumberOfLayers(); indexLayer++) {
            ...
            if (considerTrafficInOversubscribedLinksAsLost.getBoolean()) for (Demand d : netPlan.getDemands(layer))
                if (d.isTraversingOversubscribedLinks()) blocked_d.set(d.getIndex(), d.getOfferedTraffic());
            if (maximumE2ELatencyMs.getDouble() > 0) for (Demand d : netPlan.getDemands(layer))
                if (d.getWorseCasePropagationTimeInMs() > maximumE2ELatencyMs.getDouble())
                    blocked_d.set(d.getIndex(), d.getOfferedTraffic());
            ...
            if (considerTrafficInOversubscribedLinksAsLost.getBoolean())
                for (MulticastDemand d : netPlan.getMulticastDemands(layer))
                    if (d.isTraversingOversubscribedLinks()) blocked_md.set(d.getIndex(), d.getOfferedTraffic());
            if (maximumE2ELatencyMs.getDouble() > 0) for (MulticastDemand d : netPlan.getMulticastDemands(layer))
                if (d.getWorseCasePropagationTimeInMs() > maximumE2ELatencyMs.getDouble())
                    blocked_md.set(d.getIndex(), d.getOfferedTraffic());

            //			System.out.println ("****** No failure Layer  " + layer + ", blocked_d: " + blocked_d);
            final DoubleMatrix1D availabilityClassic_d = blocked_d.copy().assign(new DoubleFunction() {
                public double apply(double x) {
                    return x > PRECISION_FACTOR_blocking ? 0 : 1;
                }
            });
            final DoubleMatrix1D availabilityWeighted_d = blocked_d.copy().assign(h_d, new DoubleDoubleFunction() {
                public double apply(double x, double y) {
                    return y < PRECISION_FACTOR_hd ? 1 : 1 - x / y;
                }
            });
            final DoubleMatrix1D availabilityClassic_md = blocked_md.copy().assign(new DoubleFunction() {
                public double apply(double x) {
                    return x > PRECISION_FACTOR_blocking ? 0 : 1;
                }
            });
            final DoubleMatrix1D availabilityWeighted_md = blocked_md.copy().assign(h_md, new DoubleDoubleFunction() {
                public double apply(double x, double y) {
                    return y < PRECISION_FACTOR_hd ? 1 : 1 - x / y;
                }
            });

            availabilityClassicTotal_ld.get(layer.getIndex()).assign(availabilityClassic_d, new DoubleDoubleFunction() {
                public double apply(double x, double y) {
                    return x + pi_s0 * y;
                }
            });
            availabilityWeightedTotal_ld.get(layer.getIndex()).assign(availabilityWeighted_d, new DoubleDoubleFunction() {
                public double apply(double x, double y) {
                    return x + pi_s0 * y;
                }
            });
            availabilityClassicTotal_lmd.get(layer.getIndex()).assign(availabilityClassic_md, new DoubleDoubleFunction() {
                public double apply(double x, double y) {
                    return x + pi_s0 * y;
                }
            });
            availabilityWeightedTotal_lmd.get(layer.getIndex()).assign(availabilityWeighted_md, new DoubleDoubleFunction() {
                public double apply(double x, double y) {
                    return x + pi_s0 * y;
                }
            });
...
//			netPlan.setLinksAndNodesFailureState(upAndOversubscribedLinksSetToDown , null , null , null);
        ...
        Set<Link> linksAllLayers = new HashSet<Link>();
        for (NetworkLayer layer : auxNetPlan.getNetworkLayers()) linksAllLayers.addAll(auxNetPlan.getLinks(layer));

        for (int failureState = 1; failureState < F_s.rows(); failureState++) // first failure state (no failure) was already considered
        {
            if (!auxNetPlan.getLinksDownAllLayers().equals(initialLinksDownAllLayers) || !auxNetPlan.getNodesDown().equals(initialNodesDown))
                throw new RuntimeException("Bad");

            IntArrayList srgs_thisState = new IntArrayList();
            ...
            try {
                ...
                try {
                    ((Closeable) algorithm.getClass().getClassLoader()).close();
                } catch (Throwable e1) {
                }
            ...
            for (NetworkLayer layer : auxNetPlan.getNetworkLayers()) {
                ...
                if (considerTrafficInOversubscribedLinksAsLost.getBoolean())
                    for (Demand d : auxNetPlan.getDemands(layer))
                        if (d.isTraversingOversubscribedLinks()) blocked_d.set(d.getIndex(), d.getOfferedTraffic());
                if (maximumE2ELatencyMs.getDouble() > 0) for (Demand d : auxNetPlan.getDemands(layer))
                    if (d.getWorseCasePropagationTimeInMs() > maximumE2ELatencyMs.getDouble())
                        blocked_d.set(d.getIndex(), d.getOfferedTraffic());
                ...
                if (considerTrafficInOversubscribedLinksAsLost.getBoolean())
                    for (MulticastDemand d : auxNetPlan.getMulticastDemands(layer))
                        if (d.isTraversingOversubscribedLinks()) blocked_md.set(d.getIndex(), d.getOfferedTraffic());
                if (maximumE2ELatencyMs.getDouble() > 0) for (MulticastDemand d : auxNetPlan.getMulticastDemands(layer))
                    if (d.getWorseCasePropagationTimeInMs() > maximumE2ELatencyMs.getDouble())
                        blocked_md.set(d.getIndex(), d.getOfferedTraffic());
                final DoubleMatrix1D availabilityClassic_d = blocked_d.copy().assign(new DoubleFunction() {
                    public double apply(double x) {
...
//			System.out.println ("5. failure state: " + failureState + ", auxNetPlan.getLinksDownAllLayers(): " + auxNetPlan.getLinksDownAllLayers() + ", auxNetPlan.getNodesDown(): " + auxNetPlan.getNodesDown() + ", linksToSetDown: " + linksToSetAsDown);
    ...
    private String printReport(NetPlan np, Map<String, String> reportParameters) {
        ...
        out.append("<p>P. Pav�n Mari�o, \"Optimization of computer networks. Modeling and algorithms. A hands-on approach\", Wiley 2016</p>");
        ...
        for (NetworkLayer layer : np.getNetworkLayers()) {
            ...
            val = h_d.size() == 0 ? 0 : H;
            out.append("<li>UNICAST TRAFFIC: (Deterministic) Total offered traffic: " + df_6.format(val) + "</li>");
            if (np.getNumberOfDemands(layer) != 0) {
                val = survNoFailure_d.size() == 0 ? 0 : 1 - (survNoFailure_d.zDotProduct(h_d) / H);
                out.append("<li>UNICAST TRAFFIC: (Deterministic) Blocked traffic when no failure occurs: " + df_6.format(val) + "</li>");
                val = avTotal_d.size() == 0 ? 0 : avTotal_d.zDotProduct(h_d) / H;
                out.append("<li>UNICAST TRAFFIC: (Estimated) Network availability: " + df_6.format(val) + " - " + df_6.format(val + pi_excess) + "</li>");
                val = survTotal_d.size() == 0 ? 0 : survTotal_d.zDotProduct(h_d) / H;
                out.append("<li>UNICAST TRAFFIC: (Estimated) Network traffic survivability: " + df_6.format(val) + " - " + df_6.format(val + pi_excess) + "</li>");
                val = avTotal_d.size() == 0 ? 0 : avTotal_d.getMinLocation()[0];
                out.append("<li>UNICAST TRAFFIC: (Estimated) Worse availability among demands: " + df_6.format(val) + " - " + df_6.format(val + pi_excess) + "</li>");
                val = survTotal_d.size() == 0 ? 0 : survTotal_d.getMinLocation()[0];
                out.append("<li>UNICAST TRAFFIC: (Estimated) Worse traffic survivability among demands: " + df_6.format(val) + " - " + df_6.format(val + pi_excess) + "</li>");
            }
            final double MH = h_md.zSum();
            val = h_md.size() == 0 ? 0 : MH;
            out.append("<li>MULTICAST TRAFFIC: (Deterministic) Total offered traffic: " + df_6.format(val) + "</li>");
            if (np.getNumberOfMulticastDemands(layer) != 0) {
                val = survNoFailure_md.size() == 0 ? 0 : 1 - (survNoFailure_md.zDotProduct(h_md) / MH);
                out.append("<li>MULTICAST TRAFFIC: (Deterministic) Blocked traffic when no failure occurs: " + df_6.format(val) + "</li>");
                val = avTotal_md.size() == 0 ? 0 : avTotal_md.zDotProduct(h_md) / MH;
                out.append("<li>MULTICAST TRAFFIC: (Estimated) Network availability: " + df_6.format(val) + " - " + df_6.format(val + pi_excess) + "</li>");
                val = survTotal_md.size() == 0 ? 0 : survTotal_md.zDotProduct(h_md) / MH;
                out.append("<li>MULTICAST TRAFFIC: (Estimated) Network traffic survivability: " + df_6.format(val) + " - " + df_6.format(val + pi_excess) + "</li>");
                val = avTotal_md.size() == 0 ? 0 : avTotal_md.getMinLocation()[0];
                out.append("<li>MULTICAST TRAFFIC: (Estimated) Worse availability among demands: " + df_6.format(val) + " - " + df_6.format(val + pi_excess) + "</li>");
                val = survTotal_md.size() == 0 ? 0 : survTotal_md.getMinLocation()[0];
                out.append("<li>MULTICAST TRAFFIC: (Estimated) Worse traffic survivability among demands: " + df_6.format(val) + " - " + df_6.format(val + pi_excess) + "</li>");
            }
        }
        ...
        for (NetworkLayer layer : np.getNetworkLayers()) {
            ...
            if (np.getNumberOfMulticastDemands(layer) != 0) {
                ...
                for (MulticastDemand d : np.getMulticastDemands(layer)) {
                    String egressNodesInfo = "";
                    for (Node n : d.getEgressNodes()) egressNodesInfo += n.getIndex() + "(" + n.getName() + ") ";
