...
public class Online_evProc_multidomainRoutingPrimalDecomp extends IEventProcessor {
    ...
    public void processEvent(NetPlan currentNetPlan, SimEvent event) {
        ...
        switch (event.getEventType()) {
            case ROUTING_DOMAINRECOMPUTE: {
                ...
                for (int n = 0; n < N; n++)
                    for (int d = 0; d < D; d++)
                        if ((Math.abs(outFR_dn.get(d, n) - 1) > 1e-3) && (Math.abs(outFR_dn.get(d, n)) > 1e-3))
                            throw new RuntimeException("Bad. outFR_dn:  " + outFR_dn);

				/* In some rare occasions, the routing may create loops. These routings are not applied to the network */
                try {
                    this.currentNetPlan.setForwardingRules(new_fde);
                } catch (ClosedCycleRoutingException e) {
                    System.out.println("The routing after this domain update would produce a closed loop. It is not applied.");
                }
                ...
                if (time > 0) {
                    ...
                    if (blockedTraffic.getMaxLocation()[0] > 1e-3)
                        System.out.println("Some traffic is lost. Sum blocked traffic of the demands: " + blockedTraffic.zSum());
                    if (linkOversubscription.getMaxLocation()[0] > 1e-3)
                        System.out.println("Some links are ovsersubscribed. Sum oversubscription traffic in the links: " + linkOversubscription.zSum());
    ...
    private DoubleMatrix2D projectMaster(DoubleMatrix2D can_mte, DoubleMatrix2D old_mte) {
        ...
        if (old_mte != null)
            op.setInitialSolution("m_te", new DoubleMatrixND(old_mte.viewSelection(allT, frontierLinkGlobalIndex_ef).copy()));
        ...
        if (!op.solutionIsOptimal())
            return old_mte.copy(); // sometimes IPOPT is unstable. do not stop the algorithm then
