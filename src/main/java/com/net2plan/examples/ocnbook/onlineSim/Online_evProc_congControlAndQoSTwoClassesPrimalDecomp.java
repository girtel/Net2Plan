...
public class Online_evProc_congControlAndQoSTwoClassesPrimalDecomp extends IEventProcessor {
    ...
    public void initialize(NetPlan currentNp, Map<String, String> algorithmParameters, Map<String, String> simulationParameters, Map<String, String> net2planParameters) {
        ...
        if (currentNetPlan.getVectorLinkCapacity().getMaxLocation()[0] < mac_minCapacity_1.getDouble() + mac_minCapacity_2.getDouble())
            throw new Net2PlanException("Minimum link capacities for each class are too high for the available link capacities");
        ...
        for (Node n1 : this.currentNetPlan.getNodes())
            for (Node n2 : this.currentNetPlan.getNodes())
                if (n1 != n2) {
                    final Demand d1 = this.currentNetPlan.addDemand(n1, n2, 0.0, null);
                    d1.setAttribute("type", "1");
                    demandType.set(d1.getIndex(), 1);
                    final Demand d2 = this.currentNetPlan.addDemand(n1, n2, 0.0, null);
                    d1.setAttribute("type", "2");
                    demandType.set(d2.getIndex(), 2);
                }
        ...
        for (Route r : currentNp.getRoutes())
            r.setCarriedTraffic(cc_control_minHd.getDouble(), cc_control_minHd.getDouble());
    ...
    public void processEvent(NetPlan currentNetPlan, SimEvent event) {
        ...
        switch (event.getEventType()) {
            case MAC_UPDATE_WAKEUPTOUPDATE: {
                ...
                if (mac_u1.get(eMe.getIndex()) + mac_u2.get(eMe.getIndex()) > eMe.getCapacity() + PRECISIONFACTOR)
                ...
                if (t > this.mac_simulation_maxNumberOfUpdateIntervals.getDouble() * this.mac_update_averageInterUpdateTime.getDouble()) {
                    this.endSimulation();
                }
			...
			/* Send the events of the signaling information messages to all the nodes */
                for (Route route : eMe.getTraversingRoutes()) {
                    if (rng.nextDouble() < this.cc_signaling_signalingLossProbability.getDouble())
                        continue; // the signaling may be lost => lost to all demands
            ...
            case CC_UPDATE_WAKEUPTOUPDATE: {
                ...
                if (t > this.cc_simulation_maxNumberOfUpdateIntervals.getDouble() * this.cc_update_averageInterUpdateTime.getDouble()) {
                    this.endSimulation();
                }
    ...
    public String finish(StringBuilder st, double simTime) {
        ...
        for (Demand d : copyInitialNetPlan.getDemands())
            if (d.getAttribute("type").equals("1")) jom_hd1.set(counter_1++, d.getOfferedTraffic());
            else jom_hd2.set(counter_2++, d.getOfferedTraffic());
        for (Link e : copyInitialNetPlan.getLinks()) {
            jom_ue1.set(e.getIndex(), Double.parseDouble(e.getAttribute("u_1")));
            jom_ue2.set(e.getIndex(), Double.parseDouble(e.getAttribute("u_2")));
        }
    ...
    private Triple<Double, Pair<DoubleMatrix1D, DoubleMatrix1D>, Pair<DoubleMatrix1D, DoubleMatrix1D>> computeObjectiveFunctionFromNetPlan(NetPlan np) {
        ...
        for (Demand d : np.getDemands()) {
            ...
            for (Link e : seqLinks) {
                ...
                if (type == 1) y1.set(index_e, y1.get(index_e) + h_d);
                else y2.set(index_e, y2.get(index_e) + h_d);
            }
            if (type == 1) {
