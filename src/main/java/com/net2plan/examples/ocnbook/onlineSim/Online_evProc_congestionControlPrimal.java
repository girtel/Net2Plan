...
public class Online_evProc_congestionControlPrimal extends IEventProcessor {
    ...
    public void initialize(NetPlan currentNetPlan, Map<String, String> algorithmParameters, Map<String, String> simulationParameters, Map<String, String> net2planParameters) {
        ...
        for (Demand d : currentNetPlan.getDemands()) // carry the demand traffic
        {
            ...
            for (Route r : routes)
                r.setCarriedTraffic(d.getOfferedTraffic() / routes.size(), d.getOfferedTraffic() / routes.size());
        }
		
		/* Set the initial prices in the links */
        this.control_priceFirstOrder_e = DoubleFactory1D.dense.make(E);
        for (Link e : this.currentNetPlan.getLinks())
            control_priceFirstOrder_e.set(e.getIndex(), computeFirstOrderPriceFromNetPlan(e));
        this.control_priceSecondOrder_e = DoubleFactory1D.dense.make(E);
        for (Link e : this.currentNetPlan.getLinks())
            control_priceSecondOrder_e.set(e.getIndex(), computeSecondOrderPriceFromNetPlan(e));
    ...
    public void processEvent(NetPlan currentNetPlan, SimEvent event) {
        ...
        switch (event.getEventType()) {
			...
			/* compute the demand price as weighted sum in the routes of route prices  */
                ...
                if (Math.abs(demandCarriedTraffic - dMe.getOfferedTraffic()) > 1E-3)
                    throw new RuntimeException("Not all the traffic is carried. demandCarriedTraffic: " + demandCarriedTraffic);
			...
			/* compute the new h_d */
                ...
                for (Route r : routes)
                    r.setCarriedTraffic(dMe.getOfferedTraffic() / routes.size(), dMe.getOfferedTraffic() / routes.size());
