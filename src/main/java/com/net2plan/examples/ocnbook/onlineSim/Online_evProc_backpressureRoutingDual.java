    ...
    public void processEvent(NetPlan currentNetPlan, SimEvent event) {
        ...
        switch (event.getEventType()) {
                ...
                for (int index_e = 0; index_e < E; index_e++) {
					...
					/* Take the queue with higher */
                        ...
                        for (int index_d = 0; index_d < D; index_d++) {
                            ...
                            if (thisQueueSize != ctlQueue_nd.get(Pair.of(a_e, d)).size())
                            ...
                            if (backpressure > highestBackpressure) {
                                indexDemandToTransmitPacket = index_d;
                                highestBackpressure = backpressure;
                            }
                        }
                        if (highestBackpressure <= routing_pressureDifferenceThreshold.getDouble())
                            break; // no more queues will be empty, not enough backpressures
                ...
                for (int index_e = 0; index_e < E; index_e++) {
                    ...
                    for (int index_d = 0; index_d < D; index_d++) {
                        ...
                        for (int cont = 0; cont < numPackets_ed; cont++) {
                            ...
                            if (this.ctlQueue_nd.get(Pair.of(a_e, d)).size() != ctlNumPacketsQueue_nd[index_ae][index_d])
                                throw new RuntimeException("Bad");
			...
			/* Update the statistics of accum number of packets in a queue */
                ...
                if (time > this.routing_maxNumberOfN2PStatisticsIntervals.getInt() * routing_statNumSchedSlotBetweenN2PRecomputing.getInt() * this.routing_fixedPacketDurationAndSchedulingInterval.getDouble()) {
                    this.endSimulation();
                }
                break;
            }
...
//		System.out.println("avTrafficGenerated_d - offeredFromN2P: " + Arrays.toString(DoubleUtils.substract(avTrafficGenerated_d, this.currentNetPlan.getVectorDemandOfferedTraffic().toArray())));
    ...
    private Pair<NetPlan, double[][]> computeOptimumSolution(boolean xdeVariablesAsFractionsOfTraffic) {
        ...
        for (Demand d : np.getDemands())
            if (d.getBlockedTraffic() > 1E-3)
                throw new RuntimeException("d: " + d + ", hd: " + d.getOfferedTraffic() + ", carried_d: " + d.getCarriedTraffic() + "... Bad");
        for (Link e : np.getLinks())
            if (e.getCarriedTrafficIncludingProtectionSegments() > e.getCapacity() + 1E-3)
                throw new RuntimeException("Bad");
    ...
    private Map<Long, Double> netPlanRouteCarriedTrafficMap(NetPlan np) {
        Map<Long, Double> res = new HashMap<Long, Double>();
        for (Route r : np.getRoutes()) res.put(r.getId(), r.getCarriedTraffic());
        return res;
    }
    ...
    private class PacketInfo {
        ...
        PacketInfo(Demand d, double creationTime) {
            this.d = d;
            this.packetCreationTime = creationTime;
            this.seqLinks = new LinkedList<Link>();
        }
    }

    private int[][] copyOf(int[][] a) {
        int[][] res = new int[a.length][];
        for (int cont = 0; cont < a.length; cont++) res[cont] = Arrays.copyOf(a[cont], a[cont].length);
        return res;
    }

    private double[][] copyOf(double[][] a) {
        double[][] res = new double[a.length][];
        for (int cont = 0; cont < a.length; cont++) res[cont] = Arrays.copyOf(a[cont], a[cont].length);
        return res;
    }

    private double[][] scaledCopyOf(int[][] a, double mult) {
        double[][] res = new double[a.length][a[0].length];
        for (int cont = 0; cont < a.length; cont++)
            for (int cont2 = 0; cont2 < a[cont].length; cont2++) res[cont][cont2] = a[cont][cont2] * mult;
        return res;
    }
}
