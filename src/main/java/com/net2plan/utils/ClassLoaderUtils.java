...
//				System.out.println ("Class " + _class + " is not assignable from: " + aux);
    ...
    private static Class getClassFromFile(File classFile, String qualifiedName) {
        ...
        if (!classFile.isFile())
            throw new Net2PlanException(classFile + " is not a valid file (e.g. it is a directory)");
...
//						System.out.print("getClassesFromJar (" + jarFile + "): " + className);
                        try {
                            _class = cl.loadClass(className); /*System.out.println("... OK"); */
                        } catch (NoClassDefFoundError e) { /*System.out.println("... ERROR"); e.printStackTrace(); */
                            continue;
                        }
