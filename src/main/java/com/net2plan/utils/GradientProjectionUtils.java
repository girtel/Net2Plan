...
public class GradientProjectionUtils {
    ...
    public static void euclideanProjection_sumEquality(DoubleMatrix1D inAndOut_x, int[] coordinatesToConsider, double vectorSum) {
        ...
        if (vectorSum == 0) {
            inAndOut_x.viewSelection(coordinatesToConsider).assign(0);
            return;
        }
        ...
        for (int numCoordsNonZero = 1; numCoordsNonZero <= K; numCoordsNonZero++) {
            double initialSum = 0;
            for (int cont = 0; cont < numCoordsNonZero; cont++) initialSum += xArray_0[orderedIndexes[cont]];
            ...
            if (lowerNonZeroCoordValue + quantityToIncreaseToAllNonZero < 0)
                continue; // this solution is not feasible, one more non-zero coordinate
            ...
            if (euclideanDistance < best_distance) {
                best_distance = euclideanDistance;
                best_numberOfCoordGreaterZero = numCoordsNonZero;
                best_quantityToIncrease = quantityToIncreaseToAllNonZero;
            }
        }
    ...
    public static void euclideanProjection_sumInequality(DoubleMatrix1D inAndOut_x, int[] coordinatesToConsider, final double xMin, final double vectorSum) {
        ...
        trueInAndOut_x.assign(new DoubleFunction() {
            public double apply(double x) {
                if (x < xMin) return xMin;
                if (x > 1E3) return vectorSum * 1E3;
                return x;
            }
        });
//		for (long k : keysToConsider) if (inAndOut_x.get(k) < xMin) inAndOut_x.put(k, xMin); else if (inAndOut_x.get(k) > 1E3) inAndOut_x.put(k, vectorSum * 1000.0);  
        ...
        while (true) {
            counter++;
            if (counter > trueInAndOut_x.size() + 1) throw new RuntimeException("WTF!!");
            ...
            for (int k = 0; k < K; k++) {
                ...
                if (this_xk > xMin + 1E-3) {
                    coordsNotInXmin.add(k);
                    minValueGreaterThanXMin = (this_xk < minValueGreaterThanXMin) ? this_xk : minValueGreaterThanXMin;
                }
            }
            ...
            for (int k : coordsNotInXmin) {
                double new_xk = trueInAndOut_x.get(k) - amountToReduceEachCoordinate;
                if (Math.abs(new_xk - xMin) < 1E-3) new_xk = xMin;
                trueInAndOut_x.set(k, new_xk);
            }
        }
    }
    ...
    public static void scaleDown_maxAbsoluteCoordinateChange(DoubleMatrix1D x0_k, DoubleMatrix1D inAndOut_x, int[] coordinateKeysToConsider, double maxAbsDifference) {
        ...
        final int K = (int) trueFinalInAndOut_x.size();
        if (trueInitial_x.size() != K) throw new Net2PlanException("Arrays must have the same size");
        double trueMaxAbsChange = 0;
        for (int k = 0; k < K; k++)
            trueMaxAbsChange = Math.max(Math.abs(trueInitial_x.get(k) - trueFinalInAndOut_x.get(k)), trueMaxAbsChange);
    ...
    public static void euclideanProjection_sumInequality(DoubleMatrix1D inAndOut_x, int[] coordinatesToConsider, final DoubleMatrix1D xMin_k, final double vectorSum) {
        ...
        final int K = (int) trueInAndOut_x.size();
		/* any links with negative p_e, are set p_e=0, any klinks with p_e > 1000, set p_e=1000 (infinities) */
        trueInAndOut_x.assign(trueXMin, new DoubleDoubleFunction() {
        ...
        while (true) {
            ...
            for (int k = 0; k < K; k++) {
                ...
                sumCurrentSolution += this_xk;
                if (distanceToXMin > 1E-3) {
                    coordsNotInXmin.add(k);
                    minDistanceToXMin = (distanceToXMin < minDistanceToXMin) ? distanceToXMin : minDistanceToXMin;
                }
            }
			/* If sums less than one => we have finished */
            if (sumCurrentSolution <= vectorSum + 1E-3) break;
            ...
            for (int k : coordsNotInXmin) {
                double new_xk = trueInAndOut_x.get(k) - amountToReduceEachCoordinate;
                if (Math.abs(new_xk - trueXMin.get(k)) < 1E-3) new_xk = trueXMin.get(k);
                trueInAndOut_x.set(k, new_xk);
            }
        }
    }
...
////	}
    ...
    public static DoubleMatrix1D regularizedProjection_sumEquality(DoubleMatrix1D a_k, int[] coordinatesToConsider, DoubleMatrix1D xMin_k, double vectorSum, double epsilon) {
        if (xMin_k == null) {
            xMin_k = DoubleFactory1D.dense.make((int) a_k.size());
        }
        ...
        for (int k = 0; k < K; k++) {
            w_k_array[k] = -true_a_k.get(k) / (2 * epsilon) - true_xMin.get(k);
        }
        ...
        if (Math.abs(pendingSum) < 1E-3) {
            DoubleMatrix1D x_k = DoubleFactory1D.dense.make((int) a_k.size());
            x_k.viewSelection(coordinatesToConsider).assign(true_xMin);
            return x_k;
        }
        ...
        for (int numNonMinCoord = 1; numNonMinCoord < K; numNonMinCoord++) {
            ...
            double accum = pendingSum;
            for (int cont = 0; cont < numNonMinCoord; cont++) accum -= w_k_array[sortIndexes_wk[cont]];
            ...
            if (smallerNonZeroDelta > -1e-5) {
                ...
                if (objFunc < bestSolutionCost) {
                    bestSolutionCost = objFunc;
                    bestSolutionXk = x_k;
                }
            }
        }
