...
public class InputParameter {
    ...
    public void initialize(double val) {
        ...
        if (!isWithinAcceptableRange())
            throw new Net2PlanException("Parameter " + this + " is not within the acceptable range");
    }
...
//		System.out.println ("o.getClass().getDeclaredFields() " +  o.getClass().getDeclaredFields());
        ...
        for (Field f : o.getClass().getDeclaredFields()) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new Net2PlanException("Could not get member " + f + " from object " + o);
            }
...
//		System.out.println ("Init c.getName (): " + c.getName ());
    ...
    public String toString() {
        ...
        if (isRunnableCode)
            return "(RunnableCode) " + memberName + (wasInitialized ? " file: " + valRunnable_file + ", classname: " + valRunnable_classname + ", parameters: " + valRunnable_parameters : "NOT READ YET");
    ...
    public void initializeFromMap(Map<String, String> map) {
        try {
            ...
            else if (isRunnableCode)
                initialize(map.get(memberName + "_file"), map.get(memberName + "_classname"), map.get(memberName + "_parameters"));
            else throw new RuntimeException("Bad");
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println(map);
            throw new Net2PlanException("Failing to convert the parameter " + memberName + " = " + map.get(memberName));
        }
    }
        ...
        if (isString) {
            ...
            if (selectOccurrence != -1) {
                ...
                for (String option : options)
                    if (valString.equals(option)) {
                        validOption = true;
                        break;
                    }
                if (!validOption)
                    return false; //throw new Net2PlanException ("Input parameter " + this.memberName + " is initialized with vaule '" + val + "' which is not a valid option :" + Arrays.toString(options));
            }
            return true;
        }
