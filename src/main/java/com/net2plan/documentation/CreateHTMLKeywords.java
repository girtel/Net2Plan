    ...
    private static void writeContents(ClassDoc[] classes, String tagName, String outputFolder) {
        ...
        for (int i = 0; i < classes.length; i++) {
            ...
            for (String keyword : StringUtils.split(keywordsString, ",")) {
                ...
                if (!keywordDescriptionMap.containsKey(keywordName))
                    throw new RuntimeException("Bad: Keyword: " + keywordName + " in algorithm " + className + ", does not exist in the description");
                if (!keywordsToAlgorithmsMap.containsKey(keywordName))
                    keywordsToAlgorithmsMap.put(keywordName, new LinkedList<String>());
        ...
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Not possible to write in keywords-all.html");
        }
...
//			new File (outputFolder + "/keywords").mkdir();
        for (Entry<String, Pair<String, String>> entry : keywordDescriptionMap.entrySet()) {
            ...
            try {
                ...
                if (examples == null)
                    System.out.println("The keyword: " + keywordName + ", appears in no examples. REMOVE???");
                ...
                e.printStackTrace();
                throw new RuntimeException("Not possible to write in " + newFileThisKeyword);
            }
    ...
    private static Object getAlgorithmAssociatedToFullyQualifiedName(String theClassName) {
        ...
        try {
            ...
            if (IAlgorithm.class.isAssignableFrom(algorithmClass))
                return (IAlgorithm) algorithmClass.getConstructor().newInstance();
            if (IReport.class.isAssignableFrom(algorithmClass))
                return (IReport) algorithmClass.getConstructor().newInstance();
            if (IEventGenerator.class.isAssignableFrom(algorithmClass))
                return (IEventGenerator) algorithmClass.getConstructor().newInstance();
            if (IEventProcessor.class.isAssignableFrom(algorithmClass))
                return (IEventProcessor) algorithmClass.getConstructor().newInstance();
            return null;
        } catch (Exception e) {
            e.printStackTrace();
