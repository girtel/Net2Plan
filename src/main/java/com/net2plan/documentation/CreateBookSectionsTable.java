...
public class CreateBookSectionsTable {
    ...
    private static void writeContents(ClassDoc[] classes, String tagName, String outputFolder) {
        for (int i = 0; i < classes.length; i++) {
            ...
            for (Tag tag : javaClass.tags()) {
                if (tag.name().equals("@" + tagName)) keywordsString += " " + tag.text() + " ";
            }
            ...
            for (String keyword : StringUtils.split(keywordsString, ",")) {
                ...
                if (keywordName.startsWith("Section ")) {
                    ...
                    if (!sectionsToAlgorithmsMap.containsKey(sectionNumber))
                        sectionsToAlgorithmsMap.put(sectionNumber, new LinkedList<String>());
                    ...
                    if (!exercisesToAlgorithmsMap.containsKey(exerciseNumber))
                        exercisesToAlgorithmsMap.put(exerciseNumber, new LinkedList<String>());
                    exercisesToAlgorithmsMap.get(exerciseNumber).add(className);
                    algorithmToExercisesMap.get(className).add(exerciseNumber);
                } else
                    throw new RuntimeException("In the tag net2plan.ocnbooksections: wrong tag value: " + keyword + ", in class: " + className + ", keywordsString: " + keywordsString);
            }
        }
        ...
        try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Not possible to write in ocnbookSections.html");
        }

    }

    public static int optionLength(String option) {
        if (option.equals("-outputFolder")) {
            return 2;
        }
        return 0;
    }
    ...
    private static String createHtml(String outputFolder) {
        ...
        s += "</tr><tr>\n";
        s += "<td><a href=\"http://www.wiley.com/WileyCDA/WileyTitle/productCd-1119013356.html\">Pablo Pav�n Mari�o, <cite>Optimization of computer networks. Modeling and algorithms. A hands-on approach</cite>, Wiley 2016.</a></td>\n";
        ...
        for (Entry<String, String> entryChapter : chapterNames.entrySet()) {
            ...
            for (String sectionNumber : sectionsToAlgorithmsMap.keySet())
                if (sectionNumber.startsWith(chapterNumber + "."))
                    sectionNumbersThisChapter.add(sectionNumber.substring(sectionNumber.indexOf(".") + 1));
            for (String exerciseNumber : exercisesToAlgorithmsMap.keySet())
                if (exerciseNumber.startsWith(chapterNumber + "."))
                    exerciseNumbersThisChapter.add(exerciseNumber.substring(exerciseNumber.indexOf(".") + 1));
