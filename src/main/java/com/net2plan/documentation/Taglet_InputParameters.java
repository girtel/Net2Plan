...
public class Taglet_InputParameters implements Taglet {
    ...
    public String toString(Tag[] tags) {
        ...
        for (int i = 0; i < tags.length; i++) {
            ...
            else if (algorithm instanceof IEventGenerator)
                result += printInformation(((IEventGenerator) algorithm).getParameters());
            else if (algorithm instanceof IEventProcessor)
                result += printInformation(((IEventProcessor) algorithm).getParameters());
    ...
    private Object getAlgorithmAssociatedToTag(Tag tag) {
        ...
        while (!javaFileObject.getName().equals("net2plan")) {
            ...
            if (javaFileObject.getParentFile() == null) break;
            else javaFileObject = javaFileObject.getParentFile();
        }
    ...
    private static String printInformation(List<Triple<String, String, String>> parameters) {
        ...
        for (Triple<String, String, String> p : parameters) {
            ...
            if (defaultString.startsWith("#boolean#")) {
                ...
                for (String value : StringUtils.split(defaultString.substring("#select#".length())))
                    st.append(value + ", ");
