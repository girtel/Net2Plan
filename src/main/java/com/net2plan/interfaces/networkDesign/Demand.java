...
public class Demand extends NetworkElement {
    ...
    public double getWorseCasePropagationTimeInMs() {
        ...
        if (layer.routingType == RoutingType.SOURCE_ROUTING) {
            ...
            List<Demand> justThisDemand = new LinkedList<Demand>();
            justThisDemand.add(this);
    ...
    public boolean isBifurcated() {
        ...
        for (Node node : netPlan.nodes) {
            int numOutLinksCarryingTraffic = 0;
            for (Link e : node.getOutgoingLinks(layer))
                if (layer.forwardingRules_x_de.get(index, e.index) > 0) numOutLinksCarryingTraffic++;
    ...
    public void coupleToUpperLayerLink(Link link) {
        ...
        if (!link.originNode.equals(this.ingressNode))
            throw new Net2PlanException("Link and demand to couple must have the same end nodes");
        if (!link.destinationNode.equals(this.egressNode))
            throw new Net2PlanException("Link and demand to couple must have the same end nodes");
        ...
        if (this.coupledUpperLayerLink != null)
            throw new Net2PlanException("Demand " + this.id + " at layer " + lowerLayer.id + " is already associated to an upper layer demand");
        ...
        if (coupling_thisLayerPair == null) {
            ...
            if (!valid)
                throw new Net2PlanException("Coupling between link " + link + " at layer " + upperLayer + " and demand " + this.id + " at layer " + lowerLayer.id + " would induce a cycle between layers");
        }
    ...
    public Link coupleToNewLinkCreated(NetworkLayer newLinkLayer) {
        ...
        try {
            ...
            if (newLink != null) newLink.remove();
            throw e;
        }
        if (ErrorHandling.isDebugEnabled()) netPlan.checkCachesConsistency();
        return newLink;
    ...
    public Pair<Set<Route>, Double> computeShortestPathRoutes(double[] costs) {
        ...
        for (Route r : cache_routes) {
            double cost = 0;
            for (Link link : r.seqLinksRealPath) cost += costs[link.index];
            if (cost < shortestPathCost) {
                shortestPathCost = cost;
                shortestRoutes.clear();
                shortestRoutes.add(r);
            }
        }
    ...
    public void remove() {
        ...
        if (layer.routingType == RoutingType.SOURCE_ROUTING)
            ...
            for (Link link : layer.links) {
                link.carriedTrafficSummingRoutesAndCarriedTrafficByProtectionSegments -= x_e.get(link.index);
                link.occupiedCapacitySummingRoutesAndCarriedTrafficByProtectionSegments -= x_e.get(link.index);
            }
        }
    ...
    public Quadruple<DoubleMatrix2D, RoutingCycleType, Double, DoubleMatrix1D> computeRoutingFundamentalMatrixDemand() {
        ...
        try {
            ...
            return Quadruple.of(null, RoutingCycleType.CLOSED_CYCLES, s_n, i_n);
        }
    ...
    void checkCachesConsistency() {
        ...
        if (layer.routingType == RoutingType.SOURCE_ROUTING) {
            ...
            for (Link e : egressNode.getIncomingLinks(layer))
                check_carriedTraffic += layer.forwardingRules_x_de.get(index, e.index);
            for (Link e : egressNode.getOutgoingLinks(layer))
                check_carriedTraffic -= layer.forwardingRules_x_de.get(index, e.index);
        }
        if (Math.abs(carriedTraffic - check_carriedTraffic) > 1e-3)
            throw new RuntimeException("Bad, carriedTraffic: " + carriedTraffic + ", check_carriedTraffic: " + check_carriedTraffic);
