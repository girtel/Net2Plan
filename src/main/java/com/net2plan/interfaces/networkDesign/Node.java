...
public class Node extends NetworkElement {
    ...
    void copyFrom(Node origin) {
        ...
        this.cache_nodeIncomingLinks.clear();
        for (Link e : origin.cache_nodeIncomingLinks)
            this.cache_nodeIncomingLinks.add(this.netPlan.getLinkFromId(e.id));
        this.cache_nodeOutgoingLinks.clear();
        for (Link e : origin.cache_nodeOutgoingLinks)
            this.cache_nodeOutgoingLinks.add(this.netPlan.getLinkFromId(e.id));
        this.cache_nodeIncomingDemands.clear();
        for (Demand d : origin.cache_nodeIncomingDemands)
            this.cache_nodeIncomingDemands.add(this.netPlan.getDemandFromId(d.id));
        this.cache_nodeOutgoingDemands.clear();
        for (Demand d : origin.cache_nodeOutgoingDemands)
            this.cache_nodeOutgoingDemands.add(this.netPlan.getDemandFromId(d.id));
        this.cache_nodeIncomingMulticastDemands.clear();
        for (MulticastDemand d : origin.cache_nodeIncomingMulticastDemands)
            this.cache_nodeIncomingMulticastDemands.add(this.netPlan.getMulticastDemandFromId(d.id));
        this.cache_nodeOutgoingMulticastDemands.clear();
        for (MulticastDemand d : origin.cache_nodeOutgoingMulticastDemands)
            this.cache_nodeOutgoingMulticastDemands.add(this.netPlan.getMulticastDemandFromId(d.id));
        this.cache_nodeSRGs.clear();
        for (SharedRiskGroup s : origin.cache_nodeSRGs) this.cache_nodeSRGs.add(this.netPlan.getSRGFromId(s.id));
        this.cache_nodeAssociatedRoutes.clear();
        for (Route r : origin.cache_nodeAssociatedRoutes)
            this.cache_nodeAssociatedRoutes.add(this.netPlan.getRouteFromId(r.id));
        this.cache_nodeAssociatedSegments.clear();
        for (ProtectionSegment s : origin.cache_nodeAssociatedSegments)
            this.cache_nodeAssociatedSegments.add(this.netPlan.getProtectionSegmentFromId(s.id));
        this.cache_nodeAssociatedulticastTrees.clear();
        for (MulticastTree t : origin.cache_nodeAssociatedulticastTrees)
            this.cache_nodeAssociatedulticastTrees.add(this.netPlan.getMulticastTreeFromId(t.id));
    }
    ...
    public double getEgressOfferedTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        double accum = 0;
        for (Demand d : cache_nodeIncomingDemands) if (d.layer.equals(layer)) accum += d.offeredTraffic;
        return accum;
    }
    ...
    public double getIngressOfferedTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (Demand d : cache_nodeOutgoingDemands) if (d.layer.equals(layer)) accum += d.offeredTraffic;
        return accum;
    }
    ...
    public double getEgressOfferedMulticastTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (MulticastDemand d : cache_nodeIncomingMulticastDemands)
            if (d.layer.equals(layer)) accum += d.offeredTraffic;
        return accum;
    }
    ...
    public double getIngressOfferedMulticastTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (MulticastDemand d : cache_nodeOutgoingMulticastDemands)
            if (d.layer.equals(layer)) accum += d.offeredTraffic;
        return accum;
    }
    ...
    public double getEgressCarriedTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (Demand d : cache_nodeIncomingDemands) if (d.layer.equals(layer)) accum += d.carriedTraffic;
        return accum;
    }
    ...
    public double getIngressCarriedTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (Demand d : cache_nodeOutgoingDemands) if (d.layer.equals(layer)) accum += d.carriedTraffic;
        return accum;
    }
    ...
    public double getEgressCarriedMulticastTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        double accum = 0;
        for (MulticastDemand d : cache_nodeIncomingMulticastDemands)
            if (d.layer.equals(layer)) accum += d.carriedTraffic;
        return accum;
    }
    ...
    public Set<Link> getIncomingLinks(NetworkLayer... optionalLayerParameter) {
        ...
        for (Link e : cache_nodeIncomingLinks) if (e.layer.equals(layer)) res.add(e);
        return res;
    }
    ...
    public Set<Link> getOutgoingLinks(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        Set<Link> res = new HashSet<Link>();
        for (Link e : cache_nodeOutgoingLinks) if (e.layer.equals(layer)) res.add(e);
        return res;
    }
    ...
    public Set<Demand> getIncomingDemands(NetworkLayer... optionalLayerParameter) {
        ...
        for (Demand e : cache_nodeIncomingDemands) if (e.layer.equals(layer)) res.add(e);
        return res;
    }
    ...
    public boolean setFailureState(boolean setAsUp) {
        ...
        aux.add(this);
        if (setAsUp) netPlan.setLinksAndNodesFailureState(null, null, aux, null);
        else netPlan.setLinksAndNodesFailureState(null, null, null, aux);
        if (ErrorHandling.isDebugEnabled()) netPlan.checkCachesConsistency();
    ...
    public Set<Route> getIncomingRoutes(NetworkLayer... optionalLayerParameter) {
        ...
        for (Demand e : cache_nodeIncomingDemands) if (e.layer.equals(layer)) res.addAll(e.cache_routes);
        return res;
    }
    ...
    public Set<MulticastTree> getIncomingMulticastTrees(NetworkLayer... optionalLayerParameter) {
        ...
        for (MulticastDemand e : cache_nodeIncomingMulticastDemands)
            if (e.layer.equals(layer)) res.addAll(e.cache_multicastTrees);
        return res;
    }
    ...
    public Set<Route> getOutgoingRoutes(NetworkLayer... optionalLayerParameter) {
        ...
        layer.checkRoutingType(RoutingType.SOURCE_ROUTING);
        Set<Route> res = new HashSet<Route>();
        for (Demand e : cache_nodeOutgoingDemands) if (e.layer.equals(layer)) res.addAll(e.cache_routes);
        return res;
    }
    ...
    public Set<MulticastTree> getOutgoingMulticastTrees(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        Set<MulticastTree> res = new HashSet<MulticastTree>();
        for (MulticastDemand e : cache_nodeOutgoingMulticastDemands)
            if (e.layer.equals(layer)) res.addAll(e.cache_multicastTrees);
        return res;
    }
    ...
    public Set<Demand> getOutgoingDemands(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        Set<Demand> res = new HashSet<Demand>();
        for (Demand e : cache_nodeOutgoingDemands) if (e.layer.equals(layer)) res.add(e);
        return res;
    }
    ...
    public Set<MulticastDemand> getIncomingMulticastDemands(NetworkLayer... optionalLayerParameter) {
        ...
        for (MulticastDemand e : cache_nodeIncomingMulticastDemands) if (e.layer.equals(layer)) res.add(e);
        return res;
    }
    ...
    public Set<MulticastDemand> getOutgoingMulticastDemands(NetworkLayer... optionalLayerParameter) {
        ...
        NetworkLayer layer = netPlan.checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        Set<MulticastDemand> res = new HashSet<MulticastDemand>();
        for (MulticastDemand e : cache_nodeOutgoingMulticastDemands) if (e.layer.equals(layer)) res.add(e);
        return res;
    }
    ...
    public void remove() {
        ...
        for (ProtectionSegment segment : new LinkedList<ProtectionSegment>(cache_nodeAssociatedSegments))
            segment.remove();
        ...
        for (MulticastDemand demand : new LinkedList<MulticastDemand>(cache_nodeIncomingMulticastDemands))
            demand.remove();
        for (MulticastDemand demand : new LinkedList<MulticastDemand>(cache_nodeOutgoingMulticastDemands))
            demand.remove();
    ...
    void checkCachesConsistency() {
        ...
        for (Demand demand : cache_nodeIncomingDemands)
            if (demand.egressNode != this) throw new RuntimeException("Bad");
        for (Demand demand : cache_nodeOutgoingDemands)
            if (demand.ingressNode != this) throw new RuntimeException("Bad");
        for (MulticastDemand demand : cache_nodeIncomingMulticastDemands)
            if (!demand.egressNodes.contains(this)) throw new RuntimeException("Bad");
        for (MulticastDemand demand : cache_nodeOutgoingMulticastDemands)
            if (demand.ingressNode != this) throw new RuntimeException("Bad");
        for (SharedRiskGroup srg : cache_nodeSRGs) if (!srg.nodes.contains(this)) throw new RuntimeException("Bad");
        for (Route route : cache_nodeAssociatedRoutes)
            if (!route.seqNodesRealPath.contains(this))
                throw new RuntimeException("Bad: " + cache_nodeAssociatedRoutes);
        for (ProtectionSegment segment : cache_nodeAssociatedSegments)
            if (!segment.seqNodes.contains(this)) throw new RuntimeException("Bad");
        for (MulticastTree tree : cache_nodeAssociatedulticastTrees)
            if (!tree.cache_traversedNodes.contains(this)) throw new RuntimeException("Bad");
    }
