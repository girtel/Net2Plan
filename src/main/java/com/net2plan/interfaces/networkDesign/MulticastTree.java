...
public class MulticastTree extends NetworkElement {
    ...
    public Set<Link> getInitialLinkSet() {
        return Collections.unmodifiableSet(this.initialSetLinksWhenWasCreated);
    }
    ...
    public void revertToInitialSequenceOfLinks() {
        this.setLinks(initialSetLinksWhenWasCreated);
    }
    ...
    void copyFrom(MulticastTree origin) {
        ...
        this.linkSet.clear();
        for (Link e : origin.linkSet) this.linkSet.add((Link) this.netPlan.getPeerElementInThisNetPlan(e));
        this.initialSetLinksWhenWasCreated.clear();
        for (Link originLink : origin.initialSetLinksWhenWasCreated)
            this.initialSetLinksWhenWasCreated.add(this.netPlan.getLinkFromId(originLink.getId()));
        this.cache_traversedNodes.clear();
        for (Node n : origin.cache_traversedNodes)
            this.cache_traversedNodes.add((Node) this.netPlan.getPeerElementInThisNetPlan(n));
        this.cache_ingressLinkOfNode.clear();
        for (Entry<Node, Link> neOrigin : origin.cache_ingressLinkOfNode.entrySet()) {
            final Node nOrigin = neOrigin.getKey();
            final Node nThis = (Node) this.netPlan.getPeerElementInThisNetPlan(nOrigin);
            final Link eOrigin = neOrigin.getValue();
            final Link eThis = (eOrigin == null) ? (Link) null : (Link) this.netPlan.getPeerElementInThisNetPlan(eOrigin);
        ...
        for (Node nOrigin : origin.pathToEgressNode.keySet()) {
            ...
            List<Link> newPath = new LinkedList<Link>();
            for (Link originLink : originPath) newPath.add((Link) this.netPlan.getPeerElementInThisNetPlan(originLink));
        ...
        for (Node nOrigin : origin.cache_egressLinksOfNode.keySet()) {
            ...
            Set<Link> newEgressLinks = new HashSet<Link>();
            for (Link originLink : originEgressLinks)
                newEgressLinks.add((Link) this.netPlan.getPeerElementInThisNetPlan(originLink));
    ...
    public List<Link> getSeqLinksToEgressNode(Node egressNode) {
        final List<Link> seqLinks = pathToEgressNode.get(egressNode);
        return (seqLinks == null) ? null : Collections.unmodifiableList(seqLinks);
    }
    ...
    public double getTreeTotalLengthInKm() {
        checkAttachedToNetPlanObject();
        double accum = 0;
        for (Link e : getLinkSet()) accum += e.getLengthInKm();
        return accum;
    }
    ...
    public void setCarriedTraffic(double newCarriedTraffic, double newOccupiedLinkCapacity) {
        ...
        if (demand.coupledUpperLayerLinks != null)
            for (Link e : demand.coupledUpperLayerLinks.values()) e.capacity = demand.carriedTraffic;
        if (ErrorHandling.isDebugEnabled()) netPlan.checkCachesConsistency();
    }
    ...
    private Triple<Set<Node>, Map<Node, Link>, Map<Node, Set<Link>>> updateCaches() {
        ...
        for (List<Link> seqLinks : pathToEgressNode.values())
            for (Link e : seqLinks) {
                ...
                else if (ingressLinkOfNode.get(linkEndNode) != e) throw new RuntimeException("Bad");
        ...
        if (!this.linkSet.equals(linkSetFromIngressLinks))
            throw new RuntimeException("Bad: linkSet: " + linkSet + ", linkSetFromIngress: " + linkSetFromIngressLinks);
    ...
    private boolean isLooplessPath(NetPlan netPlan, NetworkLayer layer, List<Link> seqLinks) {
        ...
        Set<Node> traversedNodes = new HashSet<Node>();
        traversedNodes.add(pathInitialNode);
        Node previousLinkOutNode = pathInitialNode;
        for (Link e : seqLinks) {
            e.checkAttachedToNetPlanObject(netPlan);
            if (!e.layer.equals(layer))
                throw new Net2PlanException("The link " + e + " does not belong to the multicast tree layer");
    ...
    void checkCachesConsistency() {
        ...
        for (Node node : cache_traversedNodes)
            if (!node.cache_nodeAssociatedulticastTrees.contains(this)) throw new RuntimeException("Bad");
        ...
        if (shouldBeUp) for (Node n : cache_traversedNodes)
            if (!n.isUp) {
                shouldBeUp = false;
                break;
            }
