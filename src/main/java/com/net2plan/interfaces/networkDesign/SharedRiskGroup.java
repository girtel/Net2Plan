...
public class SharedRiskGroup extends NetworkElement {
    ...
    void copyFrom(SharedRiskGroup origin) {
        ...
        this.links.clear();
        for (Link e : origin.links) this.links.add(this.netPlan.getLinkFromId(e.id));
        this.nodes.clear();
        for (Node n : origin.nodes) this.nodes.add(this.netPlan.getNodeFromId(n.id));
    }
    ...
    public Set<Link> getAffectedLinks() {
        ...
        res.addAll(links);
        for (Node n : nodes) {
            res.addAll(n.cache_nodeIncomingLinks);
            res.addAll(n.cache_nodeOutgoingLinks);
        }
        return res;
    }
    ...
    public Set<MulticastTree> getAffectedMulticastTrees(NetworkLayer layer) {
        ...
        for (Node n : nodes)
            for (MulticastTree t : n.cache_nodeAssociatedulticastTrees) if (t.layer.equals(layer)) res.add(t);
        return res;
    }
    ...
    public Set<ProtectionSegment> getAffectedProtectionSegments(NetworkLayer layer) {
        Set<ProtectionSegment> res = new HashSet<ProtectionSegment>();
        for (Link e : links)
            for (ProtectionSegment s : e.cache_traversingSegments) if (s.layer.equals(layer)) res.add(s);
        for (Node n : nodes)
            for (ProtectionSegment s : n.cache_nodeAssociatedSegments) if (s.layer.equals(layer)) res.add(s);
        return res;
    }
    ...
    public void addLink(Link link) {
        ...
        if (link instanceof ProtectionSegment)
            throw new Net2PlanException("Protections segments cannot be added as links of a SRG");
