                ...
                case XMLEvent.START_ELEMENT:
                    ...
                    switch (startElementName) {
                        ...
                        case "layerCouplingMulticastDemand":
                            ...
                            Set<Link> setLinksToCouple = new HashSet<Link>();
                            for (long oldLinkId : upperLayerLinkIds) setLinksToCouple.add(mapOldId2Link.get(oldLinkId));
...
//		netPlan.nextSegmentId.put(layerId, segmentId);
        ...
        for (long linkId : seqLinks) newSeqLinks.add(mapOldId2Link.get(linkId));
    ...
    protected void parseNode(NetPlan netPlan, XMLStreamReader2 xmlStreamReader) throws XMLStreamException {
        ...
        boolean isUp = true;
        try {
            isUp = xmlStreamReader.getAttributeAsBoolean(xmlStreamReader.getAttributeIndex(null, "isUp"));
        } catch (Exception e) {
        }
...
//		netPlan.nextDemandId.put(layerId, demandId);
    ...
    protected void parseRoute(NetPlan netPlan, long layerId, XMLStreamReader2 xmlStreamReader) throws XMLStreamException {
        ...
        try {
            carriedTrafficIfNotFailing = xmlStreamReader.getAttributeAsDouble(xmlStreamReader.getAttributeIndex(null, "carriedTrafficIfNotFailing"));
        } catch (Exception e) {
        }
        double occupiedLinkCapacityIfNotFailing = occupiedCapacity;
        try {
            occupiedLinkCapacityIfNotFailing = xmlStreamReader.getAttributeAsDouble(xmlStreamReader.getAttributeIndex(null, "occupiedLinkCapacityIfNotFailing"));
        } catch (Exception e) {
        }
        ...
        try {
            currentSeqLinksAndProtectionSegments = LongUtils.toList(xmlStreamReader.getAttributeAsLongArray(xmlStreamReader.getAttributeIndex(null, "currentSeqLinksAndProtectionSegments")));
        } catch (Exception e) {
        }
...
//		netPlan.nextRouteId.put(layerId, routeId);
        List<Link> initialSeqLinksWhenCreated_link = new LinkedList<Link>();
        for (long linkId : initialSeqLinksWhenCreated) initialSeqLinksWhenCreated_link.add(mapOldId2Link.get(linkId));
        ...
        for (long linkId : currentSeqLinksAndProtectionSegments)
            ...
            else if (mapOldId2ProtectionSegment.get(linkId) != null)
                seqLinksAndProtectionSegments_link.add(mapOldId2ProtectionSegment.get(linkId));
            else
                throw new Net2PlanException("Error parsing route information (current sequence of links and protection segments)");
        ...
        for (long segmentId : backupSegmentList)
            newRoute.addProtectionSegment(mapOldId2ProtectionSegment.get(segmentId));
...
//							long layerId = xmlStreamReader.getAttributeAsLong(xmlStreamReader.getAttributeIndex(null, "layerId"));
    ...
    protected void parseMulticastDemand(NetPlan netPlan, long layerId, XMLStreamReader2 xmlStreamReader) throws XMLStreamException {
        ...
        for (long nodeId : egressNodeIds) newEgressNodes.add(mapOldId2Node.get(nodeId));
    ...
    protected void parseMulticastTree(NetPlan netPlan, long layerId, XMLStreamReader2 xmlStreamReader) throws XMLStreamException {
        ...
        Set<Link> initialSetLinks_link = new HashSet<Link>();
        for (long linkId : initialSetLinksWhenWasCreated) initialSetLinks_link.add(mapOldId2Link.get(linkId));
        Set<Link> currentSetLinks_link = new HashSet<Link>();
        for (long linkId : currentSetLinks) currentSetLinks_link.add(mapOldId2Link.get(linkId));
