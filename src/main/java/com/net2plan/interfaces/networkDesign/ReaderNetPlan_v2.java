...
//		netPlan.nextSegmentId.put(layerId, segmentId);
        ...
        for (long linkId : seqLinks) newSeqLinks.add(mapOldId2Link.get(Pair.of(layerId, linkId)));
...
//		netPlan.nextRouteId.put(layerId, routeId);
        ...
        for (long segmentId : backupSegmentList)
            newRoute.addProtectionSegment(mapOldId2ProtectionSegment.get(Pair.of(layerId, segmentId)));
