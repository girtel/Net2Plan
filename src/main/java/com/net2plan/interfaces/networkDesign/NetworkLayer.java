...
//		for (ProtectionSegment s : origin.protectionSegments) this.protectionSegments.add((ProtectionSegment) this.netPlan.getPeerElementInThisNetPlan (s));
        ...
        this.cache_linksDown.clear();
        for (Link e : origin.cache_linksDown) this.cache_linksDown.add(this.netPlan.getLinkFromId(e.id));
        this.cache_coupledLinks.clear();
        for (Link e : origin.cache_coupledLinks) this.cache_coupledLinks.add(this.netPlan.getLinkFromId(e.id));
        this.cache_coupledDemands.clear();
        for (Demand d : origin.cache_coupledDemands) this.cache_coupledDemands.add(this.netPlan.getDemandFromId(d.id));
        this.cache_coupledMulticastDemands.clear();
        for (MulticastDemand d : origin.cache_coupledMulticastDemands)
            this.cache_coupledMulticastDemands.add(this.netPlan.getMulticastDemandFromId(d.id));
        this.cache_routesDown.clear();
        for (Route r : origin.cache_routesDown) this.cache_routesDown.add(this.netPlan.getRouteFromId(r.id));
        this.cache_multicastTreesDown.clear();
        for (MulticastTree t : origin.cache_multicastTreesDown)
            this.cache_multicastTreesDown.add(this.netPlan.getMulticastTreeFromId(t.id));
        this.cache_segmentsDown.clear();
        for (ProtectionSegment s : origin.cache_segmentsDown)
            this.cache_segmentsDown.add(this.netPlan.getProtectionSegmentFromId(s.id));
...
//
    ...
    public void checkRoutingType(RoutingType routingType) {
        if (this.routingType != routingType)
            throw new Net2PlanException("Routing type of layer " + this + " must be " + routingType);
    }
...
//		System.out.println ("updateHopByHopRoutingDemand demand: " + demand + ", this demand x_e: " + forwardingRules_x_de.viewRow(demand.index));
    ...
    void checkCachesConsistency() {
        ...
        if (routingType == RoutingType.HOP_BY_HOP_ROUTING) {
            if ((forwardingRules_f_de.rows() != D) || (forwardingRules_f_de.columns() != E))
                throw new RuntimeException("Bad");
            if ((forwardingRules_x_de.rows() != D) || (forwardingRules_x_de.columns() != E))
                throw new RuntimeException("Bad");
            if ((forwardingRules_Aout_ne.rows() != N) || (forwardingRules_Aout_ne.columns() != E))
                throw new RuntimeException("Bad");
            if ((forwardingRules_Ain_ne.rows() != N) || (forwardingRules_Ain_ne.columns() != E))
                throw new RuntimeException("Bad");
            if ((D > 0) && (E > 0)) {
        ...
        for (Link link : cache_coupledLinks)
            if ((link.coupledLowerLayerDemand == null) && (link.coupledLowerLayerMulticastDemand == null))
                throw new RuntimeException("Bad");
        for (Demand demand : cache_coupledDemands)
            if (demand.coupledUpperLayerLink == null) throw new RuntimeException("Bad");
        for (MulticastDemand demand : cache_coupledMulticastDemands)
            if (demand.coupledUpperLayerLinks == null) throw new RuntimeException("Bad");
