...
public class ProtectionSegment extends Link {
    ...
    ProtectionSegment(NetPlan netPlan, long id, int index, List<Link> seqLinks, double reservedCapacity, AttributeMap attributes) {
        ...
        this.seqNodes = new LinkedList<Node>();
        seqNodes.add(this.getOriginNode());
        for (Link e : seqLinks) seqNodes.add(e.getDestinationNode());
    ...
    void copyFrom(ProtectionSegment origin) {
        ...
        this.seqLinks.clear();
        for (Link e : origin.seqLinks) this.seqLinks.add(this.netPlan.getLinkFromId(e.id));
        this.seqNodes.clear();
        for (Node n : origin.seqNodes) this.seqNodes.add(this.netPlan.getNodeFromId(n.id));
        this.associatedRoutesToWhichIAmPotentialBackup.clear();
        for (Route r : origin.associatedRoutesToWhichIAmPotentialBackup)
            this.associatedRoutesToWhichIAmPotentialBackup.add(this.netPlan.getRouteFromId(r.id));
        ...
        for (SharedRiskGroup srg : origin.cache_srgs) this.cache_srgs.add(this.netPlan.getSRGFromId(srg.id));
        this.cache_traversingTrees = new HashSet<MulticastTree>();
        if (!origin.cache_traversingTrees.isEmpty()) throw new RuntimeException("Bad");
        this.cache_traversingSegments = new HashSet<ProtectionSegment>();
        if (!origin.cache_traversingSegments.isEmpty()) throw new RuntimeException("Bad");
        this.coupledLowerLayerDemand = null;
        if (origin.coupledLowerLayerDemand != null) throw new RuntimeException("Bad");
        this.coupledLowerLayerMulticastDemand = null;
        if (origin.coupledLowerLayerMulticastDemand != null) throw new RuntimeException("Bad");
    }
    ...
    public Map<Pair<Demand, Link>, Double> getForwardingRules() {
        throw new Net2PlanException("This method cannot be called for ProtectionSegments");
    }
     ...
     * This method cannot be called for protection segments (raises an exception)
     */
    public Demand coupleToNewDemandCreated(NetworkLayer newLinkLayer) {
        throw new Net2PlanException("This method cannot be called for ProtectionSegments");
    }
...
//
    ...
    void checkCachesConsistency() {
        ...
        for (Node node : seqNodes)
            if (!node.cache_nodeAssociatedSegments.contains(this)) throw new RuntimeException("Bad");
        for (Route route : associatedRoutesToWhichIAmPotentialBackup)
            if (!route.potentialBackupSegments.contains(this)) throw new RuntimeException("Bad");
        ...
        if (shouldBeUp) for (Node n : seqNodes)
            if (!n.isUp) {
                shouldBeUp = false;
                break;
            }
        ...
        if (!shouldBeUp) {
            if (carriedTrafficSummingRoutesAndCarriedTrafficByProtectionSegments != 0)
                throw new RuntimeException("Bad");
        }
        double checkCarryingTraffic = 0;
        double checkOccupiedSegmentCapacity = 0;
        for (Entry<Route, Integer> entry : this.cache_traversingRoutes.entrySet()) {
            final Route r = entry.getKey();
            final Integer times = entry.getValue();
            if (times < 1) throw new RuntimeException("Bad");
        ...
        if (Math.abs(checkOccupiedSegmentCapacity - this.occupiedCapacitySummingRoutesAndCarriedTrafficByProtectionSegments) > 1e-3)
            throw new RuntimeException("Bad");
    }
