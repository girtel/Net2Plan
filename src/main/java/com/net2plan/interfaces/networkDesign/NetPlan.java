...
//		System.out.println ("End NetPlan(File file): " + netPlan + " ----------- ");
    ...
    public NetworkLayer addLayerFrom(NetworkLayer origin) {
        ...
        for (ProtectionSegment originSegment : origin.protectionSegments) {
            ...
            for (Link originLink : originSegment.seqLinks) newSeqLinks.add(newLayer.links.get(originLink.index));
        ...
        for (Route originRoute : origin.routes) {
            ...
            for (Link originLink : originRoute.seqLinksRealPath)
                newSeqLinksRealPath.add(newLayer.links.get(originLink.index));
    ...
    public ProtectionSegment addProtectionSegment(List<Link> sequenceOfLinks, double reservedCapacity, Map<String, String> attributes) {
        ...
        if (sequenceOfLinks.size() != new HashSet<Link>(sequenceOfLinks).size())
            throw new Net2PlanException("Protection segments cannot traverse the same link more than once");
        ...
        for (Link link : sequenceOfLinks) {
            link.cache_traversingSegments.add(segment);
            if (!link.isUp) segmentIsUp = false;
        }
        for (Node node : segment.seqNodes) {
            node.cache_nodeAssociatedSegments.add(segment);
            if (!node.isUp) segmentIsUp = false;
        }
    ...
    public Route addRoute(Demand demand, double carriedTraffic, double occupiedLinkCapacity, List<Link> sequenceOfLinks, Map<String, String> attributes) {
        ...
        for (Node node : route.seqNodesRealPath) {
            ...
            for (int firstPathIndex = 0; firstPathIndex < P_d - 1; firstPathIndex++) {
                ...
                for (int secondPathIndex = firstPathIndex + 1; secondPathIndex < P_d; secondPathIndex++) {
                    ...
                    if (linkDisjoint) {
                        ...
                        for (SharedRiskGroup srg : SRGUtils.getAffectingSRGs(secondPath))
    ...
    public Map<Demand, List<List<Link>>> computeUnicastCandidatePathList(NetworkLayer layer, double[] costs, String... paramValuePairs) {
        ...
        layer.checkRoutingType(RoutingType.SOURCE_ROUTING);
        if (costs != null) if (costs.length != layer.links.size())
        ...
        if (numParameters * 2 != paramValuePairs.length)
        ...
        for (Demand d : layer.demands)
            cpl.put(d, GraphUtils.getKLooplessShortestPaths(nodes, layer.links, d.ingressNode, d.egressNode, linkCostMap, K, maxLengthInKm, maxNumHops, maxPropDelayInMs, maxRouteCost, maxRouteCostFactorRespectToShortestPath, maxRouteCostRespectToShortestPath));
        return cpl;
    }
    ...
    public void addRoutesAndProtectionSegmentFromCandidate11PathList(Map<Demand, List<Pair<List<Link>, List<Link>>>> cpl11) {
        ...
        try {
            ...
            for (Route r : routes) r.remove();
            for (ProtectionSegment s : segments) s.remove();
            throw e;
        }
        if (ErrorHandling.isDebugEnabled()) this.checkCachesConsistency();
    }
...
//
     ...
     */
    void checkExistsProtectionSegment(long id) {
        if (cache_id2ProtectionSegmentMap.get(id) == null)
            throw new Net2PlanException("ProtectionSegment of id: " + id + " does not exist");
    }
        ...
        if (layer != null) {
            ...
            if (e instanceof Demand) {
                if (e != layer.demands.get(e.index))
                    throw new Net2PlanException("Element " + e + " is not the same object as the one in netPlan object.");
            } else if (e instanceof ProtectionSegment) {
                if (e != layer.protectionSegments.get(e.index))
                    throw new Net2PlanException("Element " + e + " is not the same object as the one in netPlan object.");
...
//		System.out.println ("************** En el copy () *********************************************************");
    ...
    public double getDemandTotalBlockedTraffic(NetworkLayer... optionalLayerParameter) {
        NetworkLayer layer = checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (Demand d : layer.demands) accum += Math.max(0, d.offeredTraffic - d.carriedTraffic);
        return accum;
    }
    ...
    public double getDemandTotalCarriedTraffic(NetworkLayer... optionalLayerParameter) {
        NetworkLayer layer = checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (Demand d : layer.demands) accum += d.carriedTraffic;
        return accum;
    }
    ...
    public double getDemandTotalOfferedTraffic(NetworkLayer... optionalLayerParameter) {
        NetworkLayer layer = checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (Demand d : layer.demands) accum += d.offeredTraffic;
        return accum;
    }
...
//	}
    ...
    public Set<Link> getLinksOversubscribed(NetworkLayer... optionalLayerParameter) {
        ...
        Set<Link> res = new HashSet<Link>();
        for (Link e : layer.links)
            if (e.capacity < e.occupiedCapacitySummingRoutesAndCarriedTrafficByProtectionSegments) res.add(e);
        return res;
    }
    ...
    public DoubleMatrix2D getMatrixProtectionSegment2SRGAffecting(NetworkLayer... optionalLayerParameter) {
        ...
        for (ProtectionSegment r : layer.protectionSegments)
            for (Link e : r.seqLinks) for (SharedRiskGroup srg : e.cache_srgs) A_rs.set(r.index, srg.index, 1.0);
        return A_rs;
    }
    ...
    public DoubleMatrix2D getMatrixMulticastTree2SRGAffecting(NetworkLayer... optionalLayerParameter) {
        ...
        for (MulticastTree r : layer.multicastTrees)
            for (Link e : r.linkSet) for (SharedRiskGroup srg : e.cache_srgs) A_rs.set(r.index, srg.index, 1.0);
        return A_rs;
    }
    ...
    public DoubleMatrix2D getMatrixDemand2LinkTrafficCarried(NetworkLayer... optionalLayerParameter) {
        ...
        for (Route r : layer.routes)
            for (Link e : r.seqLinksRealPath)
                x_de.set(r.demand.index, e.index, x_de.get(r.demand.index, e.index) + r.carriedTraffic);
        return x_de;
    }
    ...
    public DoubleMatrix2D getMatrixLink2ProtectionSegmentAssignment(NetworkLayer... optionalLayerParameter) {
        ...
        for (ProtectionSegment r : layer.protectionSegments)
            for (Link e : r.seqLinks) delta_er.set(e.index, r.index, delta_er.get(e.index, r.index) + 1);
        return delta_er;
    }
    ...
    public DoubleMatrix2D getMatrixLink2RouteAssignment(NetworkLayer... optionalLayerParameter) {
        ...
        for (Route r : layer.routes)
            for (Link e : r.seqLinksRealPath) delta_er.set(e.index, r.index, delta_er.get(e.index, r.index) + 1);
        return delta_er;
    }
    ...
    public DoubleMatrix2D getMatrixMulticastDemand2LinkTrafficCarried(NetworkLayer... optionalLayerParameter) {
        ...
        for (MulticastTree t : layer.multicastTrees)
            for (Link e : t.linkSet)
                x_de.set(t.demand.index, e.index, x_de.get(t.demand.index, e.index) + t.carriedTraffic);
        return x_de;
    }
    ...
    public Set<MulticastDemand> getMulticastDemandsCoupled(NetworkLayer... optionalLayerParameter) {
        ...
        for (MulticastDemand demand : layer.multicastDemands)
            if (demand.coupledUpperLayerLinks != null) res.add(demand);
        return res;
    }
    ...
    public double getMulticastDemandTotalBlockedTraffic(NetworkLayer... optionalLayerParameter) {
        NetworkLayer layer = checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (MulticastDemand d : layer.multicastDemands) accum += Math.max(0, d.offeredTraffic - d.carriedTraffic);
        return accum;
    }
    ...
    public double getMulticastDemandTotalCarriedTraffic(NetworkLayer... optionalLayerParameter) {
        NetworkLayer layer = checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (MulticastDemand d : layer.multicastDemands) accum += d.carriedTraffic;
        return accum;
    }
    ...
    public double getMulticastDemandTotalOfferedTraffic(NetworkLayer... optionalLayerParameter) {
        NetworkLayer layer = checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        double accum = 0;
        for (MulticastDemand d : layer.multicastDemands) accum += d.offeredTraffic;
        return accum;
    }
    ...
    public NetworkElement getNetworkElement(long id) {
        ...
        if (e != null) return e;
        e = cache_id2ProtectionSegmentMap.get(id);
        if (e != null) return e;
    ...
    public Set<ProtectionSegment> getNodePairProtectionSegments(Node originNode, Node destinationNode, boolean returnSegmentsInBothDirections, NetworkLayer... optionalLayerParameter) {
        ...
        for (ProtectionSegment s : originNode.cache_nodeAssociatedSegments) {
            ...
            if (returnSegmentsInBothDirections)
                if ((s.originNode == destinationNode) && (s.destinationNode == originNode)) res.add(s);
        }
        return res;
    }
...
//
    ...
    public ProtectionSegment getProtectionSegment(int index, NetworkLayer... optionalLayerParameter) {
        NetworkLayer layer = checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        layer.checkRoutingType(RoutingType.SOURCE_ROUTING);
        if ((index < 0) || (index > layer.protectionSegments.size() - 1)) return null;
        else return layer.protectionSegments.get(index);
    }
     ...
     */
    public ProtectionSegment getProtectionSegmentFromId(long uid) {
        return cache_id2ProtectionSegmentMap.get(uid);
    }
    ...
    public ArrayList<Long> getProtectionSegmentIds(NetworkLayer... optionalLayerParameter) {
        ...
        layer.checkRoutingType(RoutingType.SOURCE_ROUTING);
        ArrayList<Long> res = new ArrayList<Long>();
        for (ProtectionSegment e : layer.protectionSegments) res.add(e.id);
        return res;
    }
    ...
    public DoubleMatrix1D getVectorLinkSpareCapacity(NetworkLayer... optionalLayerParameter) {
        ...
        DoubleMatrix1D res = DoubleFactory1D.dense.make(layer.links.size());
        for (Link e : layer.links)
            res.set(e.index, Math.max(0, e.capacity - e.carriedTrafficSummingRoutesAndCarriedTrafficByProtectionSegments));
        return res;
    }
    ...
    public DoubleMatrix1D getVectorLinkTotalOccupiedCapacity(NetworkLayer... optionalLayerParameter) {
        ...
        DoubleMatrix1D res = DoubleFactory1D.dense.make(layer.links.size());
        for (Link e : layer.links)
            res.set(e.index, e.occupiedCapacitySummingRoutesAndCarriedTrafficByProtectionSegments);
        return res;
    }
    ...
    public DoubleMatrix1D getVectorLinkOversubscribedTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        DoubleMatrix1D res = DoubleFactory1D.dense.make(layer.links.size());
        for (Link e : layer.links)
            res.set(e.index, Math.max(0, e.occupiedCapacitySummingRoutesAndCarriedTrafficByProtectionSegments - e.capacity));
        return res;
    }
    ...
    public DoubleMatrix1D getVectorProtectionSegmentCarriedTraffic(NetworkLayer... optionalLayerParameter) {
        ...
        for (ProtectionSegment e : layer.protectionSegments)
            res.set(e.index, e.carriedTrafficSummingRoutesAndCarriedTrafficByProtectionSegments);
        return res;
    }
    ...
    public DoubleMatrix1D computeRouteCostVector(double[] costs, NetworkLayer... optionalLayerParameter) {
        ...
        DoubleMatrix1D res = DoubleFactory1D.dense.make(layer.routes.size());
        for (Route r : layer.routes)
            for (Link link : r.seqLinksRealPath) res.set(r.index, res.get(r.index) + costs[link.index]);
        return res;
    }
    ...
    public void removeAllMulticastTreesUnused(double toleranceTrafficAndCapacityValueToConsiderUnusedTree, NetworkLayer... optionalLayerParameter) {
        ...
        for (MulticastTree t : new ArrayList<MulticastTree>(layer.multicastTrees))
            if ((t.carriedTraffic < toleranceTrafficAndCapacityValueToConsiderUnusedTree) && (t.occupiedLinkCapacity < toleranceTrafficAndCapacityValueToConsiderUnusedTree))
                t.remove();
        if (ErrorHandling.isDebugEnabled()) this.checkCachesConsistency();
    }
    ...
    public void removeAllRoutesUnused(double toleranceTrafficAndCapacityValueToConsiderUnusedRoute, NetworkLayer... optionalLayerParameter) {
        ...
        for (Route r : new ArrayList<Route>(layer.routes))
            if ((r.carriedTraffic < toleranceTrafficAndCapacityValueToConsiderUnusedRoute) && (r.occupiedLinkCapacity < toleranceTrafficAndCapacityValueToConsiderUnusedRoute))
                r.remove();
        if (ErrorHandling.isDebugEnabled()) this.checkCachesConsistency();
    }
        ...
        try {
            ...
            for (NetworkLayer layer : layers) {
                ...
                for (MulticastTree tree : layer.multicastTrees) {
                    ...
                    for (Link e : tree.initialSetLinksWhenWasCreated) linkIds.add(e.id);
                ...
                if (layer.routingType == RoutingType.SOURCE_ROUTING) {
                    ...
                    for (ProtectionSegment segment : layer.protectionSegments) {
                        ...
                        for (Link e : segment.seqLinks) seqLinks.add(e.id);
                    ...
                    for (Route route : layer.routes) {
                        ...
                        List<Long> initialSeqLinksWhenCreated = new LinkedList<Long>();
                        for (Link e : route.initialSeqLinksWhenCreated) initialSeqLinksWhenCreated.add(e.id);
                        List<Long> seqLinksAndProtectionSegments = new LinkedList<Long>();
                        for (Link e : route.seqLinksAndProtectionSegments) seqLinksAndProtectionSegments.add(e.id);
                        List<Long> backupSegmentList = new LinkedList<Long>();
                        for (ProtectionSegment e : route.potentialBackupSegments) backupSegmentList.add(e.id);
...
//		System.out.println ("affected routes: " + affectedRoutesSourceRouting);
    ...
    public double setForwardingRule(Demand demand, Link link, double splittingRatio) {
        ...
        for (Link e : link.originNode.getOutgoingLinks(layer))
            sumOutFde += layer.forwardingRules_f_de.get(demand.index, e.index);
...
//				}
    ...
    public void setVectorProtectionSegmentReservedCapacity(DoubleMatrix1D segmentCapacities, NetworkLayer... optionalLayerParameter) {
        ...
        checkIsModifiable();
        NetworkLayer layer = checkInThisNetPlanOptionalLayerParameter(optionalLayerParameter);
        if (segmentCapacities.size() != layer.protectionSegments.size())
            throw new Net2PlanException("Wrong veector size");
        if (segmentCapacities.size() > 0) if (segmentCapacities.getMinLocation()[0] < 0)
            throw new Net2PlanException("Protection segment reserved capacities must be greater or equal than zero");
        layer.checkRoutingType(RoutingType.SOURCE_ROUTING);
...
//	}
    ...
    void updateFailureStateRoutesTreesSegments(NetworkElement thisElement) {
        if (thisElement instanceof Route) {
            ...
            if (isUp) for (Node e : route.seqNodesRealPath)
                if (!e.isUp) {
                    isUp = false;
                    break;
                }
...
//				System.out.println ("down to up: route.layer.cache_routesDown: " + route.layer.cache_routesDown);
                ...
                if (route.occupiedLinkCapacity != route.occupiedLinkCapacityIfNotFailing)
                    throw new RuntimeException("Bad");
...
//				System.out.println ("up to down : route.layer.cache_routesDown: " + route.layer.cache_routesDown);
            ...
            if (isUp) // from down to up
            {
                ...
                if (tree.occupiedLinkCapacity != tree.occupiedLinkCapacityIfNotFailing)
                    throw new RuntimeException("Bad");
            ...
            boolean isUp = true;
            for (Link e : segment.seqLinks)
                if (!e.isUp) {
                    isUp = false;
                    break;
                }
            for (Node e : segment.seqNodes)
                if (!e.isUp) {
                    isUp = false;
                    break;
                }
                ...
                for (int d = 0; d < layer.demands.size(); d++) {
                    ...
                    for (int e = 0; e < layer.links.size(); e++) {
                        ...
                        for (Link inLink : a_e.getIncomingLinks(layer))
                            linkInitialNodeInTraffic += layer.forwardingRules_x_de.get(d, inLink.index);
