...
public class Route extends NetworkElement {
    ...
    Route(NetPlan netPlan, long id, int index, Demand demand, List<Link> seqLinksRealPath, AttributeMap attributes) {
        ...
        for (Link e : seqLinksRealPath) {
            if (!netPlan.equals(e.netPlan)) throw new RuntimeException("Bad");
        ...
        this.seqNodesRealPath = new LinkedList<Node>();
        seqNodesRealPath.add(demand.getIngressNode());
        for (Link e : seqLinksRealPath) seqNodesRealPath.add(e.getDestinationNode());
    }
    ...
    public List<Link> getInitialSequenceOfLinks() {
        return Collections.unmodifiableList(this.initialSeqLinksWhenCreated);
    }
    ...
    public void addProtectionSegment(ProtectionSegment segment) {
        ...
        if (!segment.layer.equals(this.layer))
            throw new Net2PlanException("The protection and the route must be of the same network layer");
    ...
    public void revertToInitialSequenceOfLinks() {
        this.setSeqLinksAndProtectionSegments(initialSeqLinksWhenCreated);
    }
    ...
    public double getLengthInKm() {
        double accum = 0;
        for (Link e : seqLinksRealPath) accum += e.lengthInKm;
        return accum;
    }
    ...
    public double getPropagationDelayInMiliseconds() {
        double accum = 0;
        for (Link e : seqLinksRealPath) accum += e.lengthInKm / e.propagationSpeedInKmPerSecond;
        return 1000 * accum;
    }
    ...
    public double getPropagationSpeedInKmPerSecond() {
        double accumDelaySeconds = 0;
        double accumLengthKm = 0;
        for (Link e : seqLinksRealPath) {
            accumDelaySeconds += e.lengthInKm / e.propagationSpeedInKmPerSecond;
            accumLengthKm += e.lengthInKm;
        }
    ...
    public void remove() {
        ...
        for (ProtectionSegment segment : potentialBackupSegments)
...
//		System.out.println ("Route: " + this + ", setCarriedTraffic: newCarriedTraffic: " + newCarriedTraffic + ", newOccupiedLinkCapacity: " + newOccupiedLinkCapacity);
        ...
        if ((newCarriedTraffic < 0) || (newOccupiedLinkCapacity < 0))
            throw new Net2PlanException("Carried traffics and occupied link capacities must be non-negative");
        ...
        if (this.isDown()) {
            ...
            this.carriedTraffic = newCarriedTraffic;
            this.occupiedLinkCapacity = newOccupiedLinkCapacity;
        }
...
//		System.out.println ("-- oldCarriedTraffic: " + oldCarriedTraffic + ",  carriedTraffic: " + carriedTraffic + ", oldOccupiedLinkCapacity: " + oldOccupiedLinkCapacity + ", occupiedLinkCapacity : " + occupiedLinkCapacity);
        ...
        for (Link linkOrSegment : seqLinksAndProtectionSegments) {
            ...
            if (linkOrSegment.carriedTrafficSummingRoutesAndCarriedTrafficByProtectionSegments < -1e-3) {
                ...
                for (Link e : seqLinksAndProtectionSegments)
                    System.out.println("Link " + e + ", is up: " + e.isUp + ", e.isDown () " + e.isDown());
                if (linkOrSegment instanceof ProtectionSegment)
                    for (Link link : ((ProtectionSegment) linkOrSegment).seqLinks)
                        System.out.println("link.carriedTrafficSummingRoutesAndCarriedTrafficByProtectionSegments: " + link.carriedTrafficSummingRoutesAndCarriedTrafficByProtectionSegments + ", is UP: " + link.isUp);
                for (Link e : seqLinksAndProtectionSegments)
                    System.out.println("Link " + e + ", is up: " + e.isUp + ", e.isDown () " + e.isDown());
            ...
            if (linkOrSegment.occupiedCapacitySummingRoutesAndCarriedTrafficByProtectionSegments < -1e-3)
                throw new RuntimeException("Bad");
            if (linkOrSegment instanceof ProtectionSegment) {
                for (Link link : ((ProtectionSegment) linkOrSegment).seqLinks) {
                    ...
                    if (link.carriedTrafficSummingRoutesAndCarriedTrafficByProtectionSegments < -1e-3)
                        throw new RuntimeException("Bad");
                    if (link.occupiedCapacitySummingRoutesAndCarriedTrafficByProtectionSegments < -1e-3)
                        throw new RuntimeException("Bad");
                }
            }
        }
    ...
    public void setSeqLinksAndProtectionSegments(List<Link> seqLinksAndProtectionSegments) {
        ...
        for (Link lps : seqLinksAndProtectionSegments)
            if (lps == null) throw new Net2PlanException("A link is the sequence is null");
                ...
                if (lps instanceof ProtectionSegment) if (!potentialBackupSegments.contains(lps))
                    throw new Net2PlanException("The protection segment is not registered as a backup for the route");
            }
        ...
        for (Link lps : seqLinksAndProtectionSegments) {
            ...
            lps.cache_traversingRoutes.put(this, numPassingTimes);
            if (lps instanceof ProtectionSegment)
                for (Link link : ((ProtectionSegment) lps).seqLinks) {
                    numPassingTimes = link.cache_traversingRoutes.get(this);
                    if (numPassingTimes == null) numPassingTimes = 1;
                    else numPassingTimes++;
                    link.cache_traversingRoutes.put(this, numPassingTimes);
                }
        }
        for (Node node : seqNodesRealPath)
    ...
    public int getNumberOfTimesLinkIsTraversed(Link e) {
        if (e instanceof ProtectionSegment)
            throw new Net2PlanException("This method is just for links, not protection segments");
        Integer num = e.cache_traversingRoutes.get(e);
    ...
    void copyFrom(Route origin) {
        ...
        this.initialSeqLinksWhenCreated.clear();
        for (Link originLink : origin.initialSeqLinksWhenCreated)
            this.initialSeqLinksWhenCreated.add(this.netPlan.getLinkFromId(originLink.getId()));
        ...
        this.seqLinksRealPath.clear();
        for (Link e : origin.seqLinksRealPath) this.seqLinksRealPath.add(this.netPlan.getLinkFromId(e.id));
        this.seqNodesRealPath.clear();
        for (Node n : origin.seqNodesRealPath) this.seqNodesRealPath.add(this.netPlan.getNodeFromId(n.id));
        this.potentialBackupSegments.clear();
        for (ProtectionSegment r : origin.potentialBackupSegments)
            this.potentialBackupSegments.add(this.netPlan.getProtectionSegmentFromId(r.id));
    ...
    void checkCachesConsistency() {
        ...
        if (seqLinksAndProtectionSegments == null)
            throw new RuntimeException("Route " + this + ", seqLinksAndProtectionSegments == null");
        if (initialSeqLinksWhenCreated == null)
            throw new RuntimeException("Route " + this + ", initialSeqLinksWhenCreated == null");
        ...
        for (Link link : seqLinksAndProtectionSegments) {
            if (link == null)
                throw new RuntimeException("Route " + this + ", seqLinksAndProtectionSegments: " + seqLinksAndProtectionSegments + ", link: " + link);
            if (link.cache_traversingRoutes == null)
                throw new RuntimeException("Route " + this + ", link: " + link + ", is segment: " + (link instanceof ProtectionSegment) + ", link.cache_traversingRoutes == null");
            if (!link.cache_traversingRoutes.containsKey(this))
                throw new RuntimeException("Bad. Route of seqLinksAndProtectionSegments: " + seqLinksAndProtectionSegments + ", seqLinksRealPath: " + seqLinksRealPath + ", in link " + link + ", it does not belong to link.cache_traversingRoutes: " + link.cache_traversingRoutes);
        }
        for (Link link : seqLinksRealPath)
            if (!link.cache_traversingRoutes.containsKey(this)) throw new RuntimeException("Bad");
        for (Node node : seqNodesRealPath)
            if (!node.cache_nodeAssociatedRoutes.contains(this)) throw new RuntimeException("Bad");
        for (ProtectionSegment segment : potentialBackupSegments)
            if (!segment.associatedRoutesToWhichIAmPotentialBackup.contains(this)) throw new RuntimeException("Bad");
        ...
        if (shouldBeUp) for (Node n : seqNodesRealPath)
            if (!n.isUp) {
                shouldBeUp = false;
                break;
            }
