...
class DemandLinkMapping {
    ...
    public void put(Demand demand, Link link) {
        ...
        if (demandSideLayer == null) demandSideLayer = demand.layer;
        else if (!demandSideLayer.equals(demand.layer)) throw new RuntimeException("Bad");
        if (linkSideLayer == null) linkSideLayer = link.layer;
        else if (!linkSideLayer.equals(link.layer)) throw new RuntimeException("Bad");
    }
    ...
    public void put(MulticastDemand demand, Set<Link> links) {
        ...
        if (linkSideLayer == null) linkSideLayer = links.iterator().next().layer;
        else if (!linkSideLayer.equals(links.iterator().next().layer)) throw new RuntimeException("Bad");
    }
    ...
    public Map<Demand, Link> getDemandMap() {
        return demandLinkMapping;
    }

    public Map<MulticastDemand, Set<Link>> getMulticastDemandMap() {
        return multicastDemandLinkMapping;
    }

    public NetworkLayer getDemandSideLayer() {
        return demandSideLayer;
    }

    public NetworkLayer getLinkSideLayer() {
        return linkSideLayer;
    }
