...
public class SimKernel implements IEventCallback {
    ...
    public void configureSimulation(Map<String, String> simulationParameters, Map<String, String> net2planParameters, IExternal eventGenerator, Map<String, String> eventGeneratorParameters, IExternal eventProcessor, Map<String, String> eventProcessorParameters) {
        ...
        if (!simulationParameters.containsKey("disableStatistics"))
            throw new Net2PlanException("'disableStatistics' parameter is not configured");
        disableStatistics = Boolean.parseBoolean(simulationParameters.get("disableStatistics"));

        if (!simulationParameters.containsKey("omitProtectionSegments"))
            throw new Net2PlanException("'omitProtectionSegments' parameter is not configured");
        ...
        if (!simulationParameters.containsKey("refreshTime"))
            throw new Net2PlanException("'refreshTime' parameter is not configured");
        ...
        if (!simulationParameters.containsKey("simEvents"))
            throw new Net2PlanException("'simEvents' parameter is not configured");
        long simEvents = Long.parseLong(simulationParameters.get("simEvents"));
        simCore.setTotalSimulationEvents(simEvents);

        if (!simulationParameters.containsKey("transitoryEvents"))
            throw new Net2PlanException("'transitoryEvents' parameter is not configured");
        ...
        if (!simulationParameters.containsKey("transitoryTime"))
            throw new Net2PlanException("'transitoryTime' parameter is not configured");
        ...
        if (!simulationParameters.containsKey("simTime"))
            throw new Net2PlanException("'simTime' parameter is not configured");
        double simTime = Double.parseDouble(simulationParameters.get("simTime"));
        simCore.setTotalSimulationTime(simTime);

        if (!getEventGeneratorClass().isAssignableFrom(eventGenerator.getClass()))
            throw new RuntimeException("Bad - Event generator is not an instance of " + getEventGeneratorClass().getName());
        if (!getEventProcessorClass().isAssignableFrom(eventProcessor.getClass()))
            throw new RuntimeException("Bad - Event processor is not an instance of " + getEventProcessorClass().getName());
...
//		currentNetPlan_view = null;
    ...
    public static Pair<NetPlan, String> runSimulation(SimKernel simKernel, NetPlan netPlan, IExternal eventGenerator, Properties customEventGeneratorParameters, IExternal eventProcessor, Properties customEventProcessorParameters, Properties customSimulatorParameters, Map<String, String> net2planParameters) {
        ...
        try {
            defaultEventGeneratorParameters = eventGenerator.getParameters();
        } catch (UnsupportedOperationException ex) {
        }

        List<Triple<String, String, String>> defaultEventProcessorParameters = null;
        try {
            defaultEventProcessorParameters = eventProcessor.getParameters();
        } catch (UnsupportedOperationException ex) {
        }
        ...
        if (simKernel.lastReason != null && !(simKernel.lastReason instanceof EndSimulationException))
            throw new RuntimeException(simKernel.lastReason);
    ...
    public void setNetPlan(NetPlan netPlan) {
        ...
        initialNetPlan = netPlan.copy();
        initialNetPlan.setModifiableState(false);
        initializeNetState();
    }
