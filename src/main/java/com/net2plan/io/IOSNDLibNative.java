        ...
        try (BufferedReader in = new BufferedReader(new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8))) {
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (bidirectionalLinks)
                    netPlan.addLinkBidirectional(originNode, destinationNode, capacity, lengthInKm, 200000, attributeMap);
            ...
            while ((line = in.readLine()) != null) {
                ...
                String name = data[0];
                long ingressNodeId = nodeName2Id.get(data[1]);
                Node ingressNode = netPlan.getNodeFromId(ingressNodeId);
                long egressNodeId = nodeName2Id.get(data[2]);
                Node egressNode = netPlan.getNodeFromId(egressNodeId);
                double offeredTraffic = Double.parseDouble(data[4]);
                Map<String, String> attributeMap = new LinkedHashMap<String, String>();
                attributeMap.put("name", name);

                if (bidirectionalDemands)
                    netPlan.addDemandBidirectional(ingressNode, egressNode, offeredTraffic, attributeMap);
            ...
            while ((line = in.readLine()) != null) {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    for (Long id : nodeName2Id.values()) nodes.add(netPlan.getNodeFromId(id));
