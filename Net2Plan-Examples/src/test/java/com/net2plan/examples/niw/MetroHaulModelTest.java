...
import com.net2plan.examples.niw.research.technoec.AssessmentBenefitsOfDynamicity;
import com.net2plan.examples.niw.research.technoec.ImporterFromTimBulkFiles_forConfTimData;
import com.net2plan.examples.niw.research.technoec.JltTecnoec1_capacityPlanningAlgorithm;
...
public class MetroHaulModelTest extends TestCase
{
	...
	public void setUp() throws Exception
	{
		...
		final Consumer<WNode> setAsRoadm = n -> n.getOpticalSwitchingArchitecture().getAsOadmGeneric().updateParameters(n.getOpticalSwitchingArchitecture().getAsOadmGeneric().getParameters().setArchitectureTypeAsBroadcastAndSelect());
		final Consumer<WNode> setAsDW = n -> n.getOpticalSwitchingArchitecture().getAsOadmGeneric().updateParameters(n.getOpticalSwitchingArchitecture().getAsOadmGeneric().getParameters().setArchitectureTypeAsFilterless());
		setAsDW.accept(n1);
		setAsDW.accept(n2);
		setAsDW.accept(n3);
		setAsDW.accept(n4);
...
//	@Test
	public void testWithCapPlanning()
	{
		...
		new ImporterFromTimBulkFiles_forConfTimData().executeAlgorithm(np, algorithmParameters, new HashMap<>());
		new WNet(np).checkConsistency();
		new JltTecnoec1_capacityPlanningAlgorithm().executeAlgorithm(np, InputParameter.getDefaultParameters(new JltTecnoec1_capacityPlanningAlgorithm().getParameters()), new HashMap<>());
		new WNet(np).checkConsistency();
