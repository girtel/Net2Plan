...
public class DimensioningPlanning_JOCN implements IAlgorithm {
	...
	public WNet runAlgorithm (NetPlan netPlan, Map<String, String> algorithmParameters){
		...
		long startLoadingTopology = System.currentTimeMillis();
		...
		long finishLoadingTopology = System.currentTimeMillis();
		double timeLoadingTopology = (double) ((finishLoadingTopology - startLoadingTopology)/1000);
		

		long startAlgorithm = System.currentTimeMillis();
		
		
		ArrayList<String> timeSCs = new ArrayList<String>();
		int numberOfOEOConversions = 0;
		...
		for (WServiceChainRequest scr : orderedServiceChainRequestsHigherToLowerTraffic)
		{
			...
			if (coreNode.equals(node)) {
				...
				finishSC = System.currentTimeMillis();
			...
			} else 
			{
				...
				while (firstFiberLinks.size() != 0) /* We decrease the list.size() in each iteration moving one step the endingNode */
				{
					...
					if (latency < serviceLatency_ms) {
						...
						numberOfOEOConversions = numberOfOEOConversions + firstFiberLinks.size();
					...
					for (WLightpath lp : lpList) {
						...
						if(lp.getB().getName().equals(endingNode.getName()) && !existsDirectLightpath(wNet, node.getName(), lp.getB().getName())) {
							...
							if (candidateLatency_ms < serviceLatency_ms) {
								...
								numberOfOEOConversions = numberOfOEOConversions + 2;
					...
					if ((propagationDelay + 2 * 0.1) < serviceLatency_ms) {// direct lightpath
						...
						numberOfOEOConversions = numberOfOEOConversions + 1;
			...
			timeSCs.add(String.valueOf(finishSC - startSC));
		}
		...
		long finishAlgorithm = System.currentTimeMillis();
		double timeAlgorithm = (double) ((finishAlgorithm - startAlgorithm)/1000);

		
		/* SAVE OPTICAL FEATURES ON FILE */
		File folder = new File ("Optic");
		if (folder.exists() && folder.isFile()) throw new Net2PlanException ("The folder is a file");
		if (!folder.exists()) folder.mkdirs();
		ArrayList<String> optic = new ArrayList<String>();
		
		optic.add(String.valueOf(lpList.size()));
		optic.add(String.valueOf(numberOfOEOConversions));
		
		writeFile (new File (folder , "Optic_" + Lmax  + ".txt") , optic);
		
		
		/* SAVE TIMES ON FILE */
		folder = new File ("Times");
		if (folder.exists() && folder.isFile()) throw new Net2PlanException ("The folder is a file");
		if (!folder.exists()) folder.mkdirs();

		writeFile (new File (folder , "SCs_ms_" + Lmax  + ".txt") , timeSCs);
		
		ArrayList<String> executionTime = new ArrayList<String>();
		
		executionTime.add(String.valueOf(timeLoadingTopology));
		executionTime.add(String.valueOf(timeAlgorithm));
		writeFile (new File (folder , "executionTime_s_" + Lmax  + ".txt") , executionTime);
	...
	private static void prepareAndAddServiceChain(WNet wNet, WServiceChainRequest scr,
		...
		//final WUserService service = wNet.getUserServicesInfo().get(scr.getId());
