...
public class ReportNiw_availability implements IReport
{
	...
	private final static String df6 (Number val) { if (val.doubleValue() == Double.MAX_VALUE) return "Inf"; if (val.doubleValue() == -Double.MAX_VALUE)return "-Inf"; return df_6.format(val.doubleValue());   }
	...
	public String executeReport(NetPlan netPlan, Map<String, String> reportParameters, Map<String, String> net2planParameters)
	{
		...
		Map<String, String> algorithmParameters = StringUtils.stringToMap(algorithmParam_alg);
		if (wNet.getSrgs().isEmpty()) throw new Net2PlanException ("No SRGs are defined");

		wNet.setFailureStateAllUp();
	...
	private String printReport(NetPlan np , Map<String,String> reportParameters)
	{
		...
		for (NetworkLayer layer : np.getNetworkLayers ())
		{
			...
			out.append ("<ul>");
			out.append ("<li>UNICAST TRAFFIC: (Deterministic) Total offered traffic: " + df6 (totalOffered_d) + "</li>");
			if (np.getNumberOfDemands (layer) != 0)
			{
				out.append ("<li>UNICAST TRAFFIC: (Deterministic) Blocked traffic when no failure occurs: " + df6 (1-weightedFractionOfOfTrafficNoFailureAv_d) + "</li>");
				...
				out.append ("<li>UNICAST TRAFFIC: (Estimated) Worst among demand latencies (ms): " + df6(np.getDemands(layer).stream().mapToDouble(d->info_d.get(d.getId()).getWcLatencyMs()).max().orElse(0.0))  + "</li>");
			}			
			out.append ("<li>MULTICAST TRAFFIC: (Deterministic) Total offered traffic: " + df6 (totalOffered_md) + "</li>");
			if (np.getNumberOfMulticastDemands(layer) != 0)
			{
				out.append ("<li>MULTICAST TRAFFIC: (Deterministic) Blocked traffic when no failure occurs: " + df6 (1-weightedFractionOfOfTrafficNoFailureAv_md) + "</li>");
				...
				out.append ("<li>MULTICAST TRAFFIC: (Estimated) Worst among demand latencies (ms): " + df6(np.getDemands(layer).stream().mapToDouble(d->info_md.get(d.getId()).getWcLatencyMs()).max().orElse(0.0))  + "</li>");
			}
		}
		...
		for (NetworkLayer layer : np.getNetworkLayers ())
		{
			...
			if (np.getNumberOfDemands(layer) != 0)
			{
				...
				final List<Function<Demand , String>> vals_d = Arrays.asList(
						...
						d->"<td>" + df6(d.getOfferedTraffic())+ "</td>",
			...
			if (np.getNumberOfLinks(layer) != 0)
			{
				...
				{
					final WNode a = new WNode (d.getOriginNode());
					final WNode b = new WNode (d.getDestinationNode());
					if (!a.isRegularNode() || !b.isRegularNode()) continue;
					out.append("<tr>");
	...
	private static String printWcAndSrgs (double val , Set<Set<SharedRiskGroup>> srgs , boolean highlight)
	{
		...
		if (val == Double.MAX_VALUE) st.append("Inf"); else st.append(df6(val));
