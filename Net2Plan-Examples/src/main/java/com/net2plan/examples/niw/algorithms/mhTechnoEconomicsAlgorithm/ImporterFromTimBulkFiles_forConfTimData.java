...
//        final List<Double> a3Weights = nodeListSheet.getWeightA3Traffic();
		...
		for (int rowAfterTitleRow = 0; rowAfterTitleRow < linksListSheet.getNumRows(); rowAfterTitleRow++)
		{
			...
			final Pair<WFiber, WFiber> pair = net.addFiber(a, b, validOpticalSlotRangesAsListPairs, distanceKm, true);
		...
		for (MH_USERSERVICES us : MH_USERSERVICES.values())
		{
			...
			for (int contNode = 0; contNode < nodeListSheet.getNumRows(); contNode++)
			{
				...
				final SortedSet<WNode> initialNodes = Sets.newTreeSet(Arrays.asList(originNode));
				final SortedSet<WNode> endNodes = new TreeSet<>(allNodes);
				final WServiceChainRequest scReqUps = net.addServiceChainRequest(initialNodes, endNodes, listVnfTypesUpstreamThisService, true, Optional.of (defaultSequenceOfExpansionFactorsRespectToInjectionUpstream), Optional.of(maxLatencyFromInitialToVnfStartUpstreamMs), us.name());
				final WServiceChainRequest scReqDowns = net.addServiceChainRequest(endNodes, initialNodes, Lists.reverse(listVnfTypesUpstreamThisService), false, Optional.of(Lists.reverse(defaultSequenceOfExpansionFactorsRespectToInjectionUpstream)), Optional.of (Lists.reverse(maxLatencyFromInitialToVnfStartUpstreamMs)), us.name());
...
//                System.out.println("Traffic type: " + us.getTrafficTypeA1A2A3());
		...
		double totalDownsTrafficA1 = net.getServiceChainRequests().stream().filter(sc -> sc.isDownstream()).filter(sc -> MH_USERSERVICES.valueOf(sc.getQosType()).isA1Traffic())
    ...
    {
		final IOadmArchitecture arch = node.getOpticalSwitchingArchitecture();
        if(arch.isPotentiallyWastingSpectrum())
            try {
        ...
        else if (arch.isNeverCreatingWastedSpectrum())
            try {
    ...
    {
    	final OadmArchitecture_generic oadm = node.getOpticalSwitchingArchitecture().getAsGenericArchitecture();
    	final Parameters oadmParameters = oadm.getParameters();
        if(option.equals("option1"))
        {
            if((NODETYPE.isMcenBbOrNot(node)) || (NODETYPE.isAmen(node)))
    		{
            	oadmParameters.setArchitectureTypeAsBroadcastAndSelect();
            	oadm.updateParameters(oadmParameters);
    		}
            else
        ...
        else if(option.equals("option2"))
        {
            ...
            if ((NODETYPE.isMcenBbOrNot(node)) || (NODETYPE.isAmen(node) && getNodeDegree(node) > 2))
            {
            	oadmParameters.setArchitectureTypeAsBroadcastAndSelect();
            	oadm.updateParameters(oadmParameters);
            }
            ...
            {
            	oadmParameters.setArchitectureTypeAsFilterless();
            	oadm.updateParameters(oadmParameters);
            }
            else
                new Net2PlanException("Node type is wrong.");
