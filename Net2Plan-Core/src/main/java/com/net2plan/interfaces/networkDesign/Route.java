...
public class Route extends NetworkElement
{
    ...
    boolean isDeepCopy(Route e2)
    {
        ...
        if (!this.currentLinksAndResourcesOccupationIfNotFailing.equals(e2.currentLinksAndResourcesOccupationIfNotFailing))
            return false;
        ...
        if (!NetPlan.isDeepCopy(this.cache_linkAndResourcesTraversedOccupiedCapIfnotFailMap, e2.cache_linkAndResourcesTraversedOccupiedCapIfnotFailMap))
            return false;
    ...
    void copyFrom(Route origin)
    {
        ...
        for (Entry<NetworkElement, Double> e : origin.cache_linkAndResourcesTraversedOccupiedCapIfnotFailMap.entrySet())
            this.cache_linkAndResourcesTraversedOccupiedCapIfnotFailMap.put(getInThisNetPlan(e.getKey()), e.getValue());
    }
    ...
    {
        return demand.isServiceChainRequest();
    }
     ...
     */
    public Set<Route> getRoutesIAmBackup()
    {
        return Collections.unmodifiableSet(this.cache_routesIAmBackUp);
    }
    ...
    {
        return Collections.unmodifiableList(this.backupRoutes);
    }
    ...
    {
        return Triple.of(initialStateCarriedTrafficIfNotFailing, Collections.unmodifiableList(initialStatePath), Collections.unmodifiableList(initialStateOccupationIfNotFailing));
    }
    ...
    {
        return !cache_routesIAmBackUp.isEmpty();
    }
    ...
    public void addBackupRoute(Route backupRoute)
    {
        ...
        if (!backupRoute.demand.equals(demand))
            throw new Net2PlanException("The backup route must be of the same demand as the primary");
        if (backupRoute.hasBackupRoutes())
            throw new Net2PlanException("A backup route cannot have backup routes itself");
    ...
    {
        double accum = 0;
        for (Link e : cache_seqLinksRealPath) accum += e.lengthInKm;
        return accum;
    }
    ...
    public int getNumberOfTimesIsTraversed(NetworkElement e)
    {
        if (e instanceof Link)
        {
            Integer num = ((Link) e).cache_traversingRoutes.get(this);
            return num == null ? 0 : num;
        } else if (e instanceof Resource)
        {
            int num = 0;
            for (NetworkElement ee : currentPath) if (ee.equals(e)) num++;
            return num;
    ...
    public void remove()
    {
        ...
        if (cache_hasLoops)
        {
            ...
            for (Route r : demand.cache_routes)
                if (r.cache_hasLoops)
                {
                    demand.routingCycleType = RoutingCycleType.OPEN_CYCLES;
                    break;
                }
        }
        ...
        if (demand.cache_worstCasePropagationTimeMs <= this.cache_propagationDelayMs)
        {
            ...
            for (Route r : demand.cache_routes)
                demand.cache_worstCasePropagationTimeMs = Math.max(demand.cache_worstCasePropagationTimeMs, r.cache_propagationDelayMs);
        }
        ...
        if (demand.cache_worstCaseLengthInKm <= this.getLengthInKm())
        {
            ...
            for (Route r : demand.cache_routes)
                demand.cache_worstCaseLengthInKm = Math.max(demand.cache_worstCaseLengthInKm, r.getLengthInKm());
        }
    ...
    public void setCarriedTraffic(double newCarriedTraffic, List<Double> linkAndResourcesOccupationInformation)
    {
        ...
        if (linkAndResourcesOccupationInformation.size() != this.currentPath.size())
            throw new Net2PlanException("Wrong vector size");
        for (double val : linkAndResourcesOccupationInformation)
            if (val < 0) throw new Net2PlanException("The occupation of a resource cannot be negative");
        ...
        for (Route r : demand.cache_routes) demand.carriedTraffic += r.getCarriedTraffic();
    ...
    public void setPath(double newCarriedTraffic, List<? extends NetworkElement> newPath, List<Double> newOccupationInformation)
    {
        ...
        if (newPath.size() != newOccupationInformation.size())
            throw new Net2PlanException("Wrong size of occupation array");
        for (Double val : newOccupationInformation)
            if (val < 0) throw new Net2PlanException("The occupation of a link/resource cannot be negative");
        ...
        for (Resource resource : this.getSeqResourcesTraversed())
            resource.removeTraversingRoute(this); // removes the current route
        ...
        this.cache_seqNodesRealPath = new LinkedList<Node>();
        cache_seqNodesRealPath.add(demand.getIngressNode());
        for (Link e : cache_seqLinksRealPath)
        {
...
<<<<<<< HEAD
    ...
    public void setSeqLinks(List<Link> seqLinks)
    {
        ...
        for (double val : currentLinksAndResourcesOccupationIfNotFailing)
            if (val != firstLinkOccup)
                throw new Net2PlanException("This method can only be used when the occupation in all the lnks is the same");
    ...
    private static List<Node> listTraversedNodes(List<Link> path)
    {
        ...
        res.add(path.get(0).originNode);
        for (Link e : path) res.add(e.getDestinationNode());
        return res;
    }
    ...
    private NetworkElement getInThisNetPlan(NetworkElement e)
    {
        ...
        if (res == null)
            throw new RuntimeException("Element of id: " + e.id + ", of class: " + e.getClass().getName() + ", does not exsit in current NetPlan");
        return res;
    }
